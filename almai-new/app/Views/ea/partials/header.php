...
<header class="bg-[#0a0a0a] border-b border-white/10 px-4 md:px-8 py-4 flex justify-between items-center fixed top-0 left-0 right-0 z-40" style="left: 0;">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs md:text-sm text-gray-500 hidden md:block"><?= $pageSubtitle ?? 'Selamat datang, ' . esc(session()->get('userName')) ?></p>
        </div>
    </div>
        ...
        <div class="relative">
            ...
            <div id="notifMenu" class="hidden absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 bg-[#111] border border-white/10 rounded-xl shadow-xl overflow-hidden z-50">
                <div class="p-3 border-b border-white/10 flex justify-between items-center">
                    ...
                    <?php if ($unreadCount > 0): ?>
                    <form method="post" action="<?= base_url('ceo/notifications/read-all') ?>"><?= csrf_field() ?><button type="submit" class="text-xs text-accent hover:underline">Tandai semua dibaca</button></form>
                    <?php endif; ?>
                </div>
                <div class="max-h-80 overflow-y-auto">
                    ...
                    <form method="post" action="<?= base_url('ceo/notifications/' . $notif['id'] . '/read') ?>" class="block px-4 py-3 hover:bg-white/5 transition border-b border-white/5 <?= $notif['is_read'] ? 'opacity-60' : '' ?>"><?= csrf_field() ?><button type="submit" class="w-full text-left">
                ...
                <a href="<?= base_url('ceo/notifications') ?>" class="block p-3 text-center text-sm text-accent hover:bg-white/5 border-t border-white/10">Lihat Semua Notifikasi</a>
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div id="profileMenu" class="hidden absolute right-0 top-full mt-2 w-48 bg-[#111] border border-white/10 rounded-xl shadow-xl overflow-hidden z-50">
                <div class="p-3 border-b border-white/10 block md:hidden">
                    <p class="font-medium text-sm"><?= esc(session()->get('userName')) ?></p>
                    <p class="text-xs text-gray-500"><?= esc(session()->get('userEmail')) ?></p>
                </div>
                <a href="<?= base_url('ceo/dashboard') ?>" class="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition text-sm">
                    <i class="fas fa-gauge-high w-4 text-gray-400"></i> Dashboard
                </a>
                <a href="<?= base_url('ceo/logout') ?>" class="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-500/10 transition text-sm text-red-400">
