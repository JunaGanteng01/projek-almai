...
<div class="mb-8 flex items-center justify-between">
    <div>
        ...
        <p class="text-gray-400">Tambahkan pengingat untuk aktivitas penting.</p>
    </div>
...
<div class="bg-gradient-to-br from-[#111] to-black border border-white/10 rounded-2xl p-6 shadow-xl max-w-2xl">
    <form action="<?= base_url('ea/quick-actions/store-reminder') ?>" method="post" class="space-y-6">
        <?= csrf_field() ?>
        
        <div>
            <label class="block text-sm font-medium text-gray-400 mb-2">Judul Reminder</label>
            <input type="text" name="title" required class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent text-white placeholder-gray-600" placeholder="Contoh: Telepon Investor">
        </div>
        ...
        <div>
            ...
            <select name="priority" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent text-white">
                ...
                <option value="kritis">Sangat Penting / Kritis</option>
            </select>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-400 mb-2">Keterangan Tambahan</label>
            <textarea name="description" rows="3" class="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-accent text-white placeholder-gray-600" placeholder="Opsional..."></textarea>
        </div>
        ...
        <div class="pt-4 border-t border-white/10 flex justify-end">
            <button type="submit" class="bg-yellow-600 hover:bg-yellow-500 text-white px-6 py-3 rounded-xl font-bold transition flex items-center shadow-lg shadow-yellow-500/20">
                <i class="fas fa-clock mr-2"></i> Set Reminder
            </button>
        </div>
    </form>
</div>
