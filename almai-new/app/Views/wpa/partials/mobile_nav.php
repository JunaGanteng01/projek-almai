...
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-black/95 backdrop-blur-xl border-t border-white/10 shadow-2xl pb-safe">
    <div class="flex items-end justify-between px-2 pb-2 pt-3">
        ...
        <!-- Home -->
        <a href="<?= base_url('wpa/dashboard') ?>" class="flex flex-col items-center gap-1 flex-1 group <?= (current_url() == base_url('wpa/dashboard')) ? 'text-accent' : 'text-gray-400' ?>">
        ...
        <!-- Layanan -->
        <a href="<?= base_url('wpa/dashboard/portofolio') ?>" class="flex flex-col items-center gap-1 flex-1 group <?= (current_url() == base_url('wpa/dashboard/layanan')) ? 'text-accent' : 'text-gray-400' ?>">
        ...
        <div class="relative flex flex-col items-center flex-1 -mt-8 pointer-events-none">
            <a href="<?= base_url('wpa/dashboard/poin') ?>" class="pointer-events-auto absolute -top-8 flex flex-col items-center">
                ...
                <span class="text-[10px] font-medium <?= (current_url() == base_url('wpa/dashboard/poin')) ? 'text-accent' : 'text-white' ?>">Poin</span>
            </a>
            <div class="h-10 w-full"></div>
        </div>

        <!-- Advokasi -->
        <a href="<?= base_url('wpa/dashboard/advokasi') ?>" class="flex flex-col items-center gap-1 flex-1 group <?= (current_url() == base_url('wpa/dashboard/advokasi')) ? 'text-accent' : 'text-gray-400' ?>">
        ...
        <!-- WPA -->
        <a href="<?= base_url('wpa/dashboard/daftar-wpa') ?>" class="flex flex-col items-center gap-1 flex-1 group <?= (current_url() == base_url('wpa/dashboard/daftar-wpa')) ? 'text-accent' : 'text-gray-400' ?>">
