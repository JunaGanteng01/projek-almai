...
<div class="space-y-8">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="<?= base_url('wpa/dashboard') ?>" class="inline-flex items-center gap-2 text-gray-400 hover:text-white transition">
                    ...
                    <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                <a href="<?= base_url('wpa/dashboard/kyc') ?>" class="inline-flex items-center justify-center gap-2 px-8 py-4 bg-red-500 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-red-600 transition shadow-lg shadow-red-500/20 text-xs">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($realReports as $adv): ?>
                        <div class="bg-[#121212] border border-white/5 rounded-[2rem] p-6 hover:border-accent/30 transition-all group relative overflow-hidden">
                            ...
                            <a href="<?= base_url('wpa/dashboard/advokasi/detail/' . $adv['id']) ?>" class="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2 relative z-10">
...
<script>
    function handleRoadmapClick(tabId, index, totalSteps) {
        const hasPurchased = <?= ($purchase || $isWpa) ? 'true' : 'false' ?>;
    ...
    function checkProAndRedirect() {
        ...
        if (!isPro) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('wpa/dashboard/kyc') ?>';
                }
            });
        } else {
            window.location.href = '<?= base_url('wpa/dashboard/advokasi/buat-laporan') ?>';
        }
    }
    ...
    function showProgramInfo() {
        Swal.fire({
            ...
            text: "Anda sebagai WPA tidak perlu membeli layanan advokasi. Akses sudah terbuka secara otomatis.",
            icon: 'info',
            confirmButtonColor: '#33E818',
        ...
        activationForm.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('<?= base_url('wpa/dashboard/advokasi/activate-license') ?>', {
                        method: 'POST',
                        body: formData,
                        headers: {
    ...
    function showSyllabus(dayId) {
        ...
        let contentHtml = `
            <div class="space-y-6 text-left">
                ...
                <div class="pt-4">
                    <a href="javascript:void(0)" onclick="checkPurchaseAndRedirect('<?= base_url('wpa/dashboard/advokasi/materi') ?>?day=${dayId}')" class="w-full py-4 bg-white text-black font-black uppercase tracking-widest rounded-2xl hover:bg-accent transition flex items-center justify-center gap-3">
        ...
        testimonialForm.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('<?= base_url('wpa/dashboard/advokasi/complete') ?>', {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
