...
<head>
    ...
    <title><?= esc($title ?? 'WPA Login - Almai') ?></title>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-8">
                <a href="<?= base_url('/') ?>" class="inline-block mb-3">
                    <img src="https://almai.id/wpa/alma.gif" alt="ALMAI" class="h-16 mx-auto">
                </a>
                <p class="text-accent font-medium text-sm">WPA Portal</p>
            </div>
            ...
            <div class="bg-[#111] border border-white/10 rounded-2xl p-8">
                <h2 class="text-2xl font-bold mb-2">WPA Login</h2>
                <p class="text-gray-500 text-sm mb-6">Masuk untuk mengelola kelas dan konten Anda</p>

                <form action="<?= base_url('wpa/login') ?>" method="post" class="space-y-4">
                    <?= csrf_field() ?>
                    <div>
                        <label class="block text-sm text-gray-400 mb-2">Email WPA</label>
                        <div class="relative">
                            <input type="email" name="email" required value="<?= old('email') ?>"
                                class="w-full bg-black border border-white/20 rounded-xl px-4 py-3 pl-12 focus:border-accent focus:outline-none" 
                                placeholder="wpa@almai.id">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="password" name="password" id="wpaPassword" required 
                                class="w-full bg-black border border-white/20 rounded-xl px-4 py-3 pl-12 focus:border-accent focus:outline-none" 
                                placeholder="••••••••">
                            <i class="fas fa-lock absolute left-4 top-1/2 -translate-y-1/2 text-gray-500"></i>
                            <button type="button" onclick="togglePassword('wpaPassword')" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white">
                    ...
                    <button type="submit" class="w-full py-4 bg-accent text-black font-bold rounded-xl hover:bg-white transition shadow-[0_0_30px_rgba(51,232,24,0.3)]">
                        <i class="fas fa-sign-in-alt mr-2"></i> Login WPA
                    </button>
                </form>
            ...
            <div class="text-center mt-6 space-y-2">
                ...
                <p class="text-gray-600 text-sm">Belum terdaftar sebagai WPA? <a href="<?= base_url('kontak') ?>" class="text-accent hover:underline">Hubungi Admin</a></p>
            </div>
        </div>
    </div>
