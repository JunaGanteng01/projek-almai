...
<div class="space-y-6">
    ...
    <form action="<?= base_url('wpa/dashboard/layanan/store') ?>" method="POST" enctype="multipart/form-data" id="wizardForm">
        <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-[#111] border border-white/10 rounded-xl p-6">
                        ...
                        <div id="singlePriceField" class="space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="price" value="0" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                                </div>
                                <div>
                                    <label class="block text-sm text-gray-400 mb-2">Harga Poin</label>
                                    <input type="number" name="poin_price" id="poinPrice" value="0" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                                </div>
                            </div>
                            <div>
                    ...
                    <div class="border-t border-white/10 pt-4 mt-6">
                         <h3 class="font-bold mb-4">Skema Referal</h3>
                         <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label class="block text-sm text-gray-400 mb-2">Bonus Cash (Referal)</label>
                                <input type="number" name="referral_user_cash" value="0" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                            </div>
                            <div>
                                <label class="block text-sm text-gray-400 mb-2">Bonus Poin (Referal)</label>
                                <input type="number" name="referral_user_poin" value="0" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm text-gray-400 mb-2">Max Kedalaman (Level)</label>
                                <input type="number" name="referral_max_depth" value="8" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                            </div>
                            <div>
                                <label class="block text-sm text-gray-400 mb-2">Distribusi (%)</label>
                                <input type="number" step="0.01" name="referral_distribution_percentage" value="50.00" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
        ...
        editor.plugins.get('FileRepository').createUploadAdapter = (loader) => {
            return {
                upload: () => loader.file.then(file => new Promise((resolve, reject) => {
                    ...
                    fetch('<?= base_url('wpa/dashboard/layanan/uploadImage') ?>', { method: 'POST', body: data })
                        .then(res => res.json())
    ...
    if (priceInput && poinInput) {
        priceInput.addEventListener('input', function() {
            poinInput.value = Math.floor(this.value / 2500);
        });
    }
