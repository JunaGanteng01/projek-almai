...
<!-- Search Bar -->
<div class="mb-6">
    <form action="<?= base_url('wpa/dashboard/daftar-wpa') ?>" method="get" class="max-w-md">
        <div class="relative">
    ...
    <?php if (empty($wpas)): ?>
        <div class="col-span-full py-20 text-center bg-[#111] border border-white/10 rounded-2xl">
            ...
            <?php if ($search): ?>
                ...
                <a href="<?= base_url('wpa/dashboard/daftar-wpa') ?>" class="inline-block mt-4 text-accent hover:underline text-sm uppercase font-bold">Lihat Semua</a>
            <?php endif; ?>
        </div>
    <?php else: ?>
        <?php foreach ($wpas as $wpa): ?>
            <div class="group bg-[#111] border border-white/10 rounded-2xl overflow-hidden hover:border-accent/50 transition-all duration-300 transform hover:-translate-y-1 shadow-lg">
                ...
                <div class="px-6 pb-6 -mt-8">
                    ...
                    <!-- Action -->
                    <a href="<?= base_url('wpa/dashboard/daftar-wpa/' . $wpa['slug']) ?>" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-xs font-bold text-white hover:bg-accent hover:text-black hover:border-accent transition-all duration-300 uppercase tracking-widest">
