    ...
    <div class="flex items-center gap-2 md:gap-4">
        ...
        <div class="relative">
            ...
            <div id="notifMenu" class="hidden absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 bg-[#111] border border-white/10 rounded-xl shadow-xl overflow-hidden z-50">
                <div class="p-3 border-b border-white/10 flex justify-between items-center">
                    ...
                    <?php if ($unreadCount > 0): ?>
                    <a href="<?= base_url('admin/notifications/read-all') ?>" class="text-xs text-accent hover:underline">Tandai semua dibaca</a>
                    <?php endif; ?>
                </div>
                <div class="max-h-80 overflow-y-auto">
                    ...
                    <a href="<?= base_url('admin/notifications/read/' . $notif['id']) ?>" class="block px-4 py-3 hover:bg-white/5 transition border-b border-white/5 <?= $notif['is_read'] ? 'opacity-60' : '' ?>">
                ...
                <a href="<?= base_url('admin/notifications') ?>" class="block p-3 text-center text-sm text-accent hover:bg-white/5 border-t border-white/10">Lihat Semua Notifikasi</a>
            </div>
        </div>
        ...
        <div class="relative">
            ...
            <div id="profileMenu" class="hidden absolute right-0 top-full mt-2 w-48 bg-[#111] border border-white/10 rounded-xl shadow-xl overflow-hidden z-50">
                ...
                <a href="<?= base_url('admin/setting') ?>" class="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition text-sm">
                    <i class="fas fa-cog w-4 text-gray-400"></i> Settings
                </a>
                <a href="<?= base_url('admin/logout') ?>" class="w-full flex items-center gap-3 px-4 py-3 hover:bg-red-500/10 transition text-sm text-red-400">
