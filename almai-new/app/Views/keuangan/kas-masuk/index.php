...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white uppercase tracking-tighter"><i class="fas fa-arrow-down text-green-400 mr-2"></i>Kas Masuk</h1>
        <p class="text-accent text-xs mt-1 font-bold">Pencatatan Pemasukan Kas Terintegrasi COA</p>
    </div>
    <div class="flex flex-wrap gap-3">
        ...
        <a href="<?= base_url('keuangan/kas-masuk/delete-all') ?>" onclick="return confirm('APAKAH ANDA YAKIN INGIN MENGHAPUS SEMUA DATA KAS MASUK?\n\nData yang sudah dihapus tidak bisa dikembalikan lagi.')" class="px-4 py-2.5 bg-red-500/20 text-red-400 border border-red-500/50 font-bold rounded-xl hover:bg-red-500/30 transition flex items-center">
            <i class="fas fa-trash-alt mr-2"></i> Hapus Semua
        </a>
        
        <a href="<?= base_url('keuangan/kas-masuk/duplikat') ?>" class="px-4 py-2.5 bg-yellow-500/20 text-yellow-500 border border-yellow-500/50 font-bold rounded-xl hover:bg-yellow-500/30 transition flex items-center" title="Kelola duplikat berdasarkan Akun Kas, Akun Pendapatan, Nominal, dan Keterangan">
            <i class="fas fa-magic mr-2"></i> Kelola Duplikat
        </a>

        <button onclick="openModal()" class="px-5 py-2.5 bg-green-500/20 text-green-400 border border-green-500/50 font-bold rounded-xl hover:bg-green-500/30 transition flex items-center">
            <i class="fas fa-plus mr-2"></i> Tambah Kas Masuk
        </button>
    </div>
</div>
    ...
    <form action="" method="GET" class="flex flex-col md:flex-row gap-4" style="color-scheme: dark;">
        ...
        <div class="flex-1">
            ...
            <select name="akun_id" class="w-full bg-[#222] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:ring-2 focus:ring-green-500/50 outline-none">
                ...
                <optgroup label="Pendapatan" class="bg-[#333] text-gray-300">
                    <?php foreach($pendapatanAccounts as $akun): ?>
                        <option class="bg-[#222] text-white" value="<?= $akun['id'] ?>" <?= (($_GET['akun_id'] ?? '') == $akun['id']) ? 'selected' : '' ?>><?= $akun['kode_akun'] ?> - <?= $akun['nama_akun'] ?></option>
                    <?php endforeach; ?>
                </optgroup>
            </select>
        ...
        <table class="w-full text-sm">
            <thead class="bg-white/5 border-b border-white/10">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-bold text-gray-400 uppercase tracking-wider">Fitur Pindah Dana</th>
            ...
            <tbody class="divide-y divide-white/5">
                <?php if (empty($transactions)): ?>
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-gray-500">Belum ada transaksi Kas Masuk.</td>
                    </tr>
                <?php else: ?>
                    <?php $no = 1; foreach ($transactions as $tx): ?>
                        <tr class="hover:bg-white/5 transition">
                            ...
                            <td class="px-4 py-3 text-right font-mono font-bold text-green-400">+ Rp <?= number_format($tx['nominal'], 0, ',', '.') ?></td>
...
<div id="kasModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4">
    <div class="bg-[#111] border border-white/10 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-white/10 flex items-center justify-between sticky top-0 bg-[#111]">
            <h2 id="modalTitle" class="text-xl font-bold text-white uppercase"><i class="fas fa-arrow-down text-green-400 mr-2"></i>Tambah Kas Masuk</h2>
        ...
        <form method="post" action="<?= base_url('keuangan/kas-masuk-keluar/save') ?>" class="p-6 space-y-5">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">PINDAH DANA (ASAL DANA)</label>
                <select id="form_akun_lawan_id" name="akun_lawan_id" required class="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-white focus:border-accent focus:outline-none transition">
                    <option value="">-- Pilih Pindah Dana --</option>
                    <optgroup label="Fitur Pindah Dana">
                        <?php foreach ($pindahDanaAccounts as $akun): ?>
                            <option value="<?= $akun['id'] ?>"><?= esc($akun['nama_akun']) ?></option>
                        <?php endforeach; ?>
                    </optgroup>
                    <?php if (!empty($pendapatanAccounts)): ?>
                    <optgroup label="Akun Pendapatan Lainnya">
                        <?php foreach ($pendapatanAccounts as $akun): ?>
                            <option value="<?= $akun['id'] ?>"><?= esc($akun['kode_akun']) ?> - <?= esc($akun['nama_akun']) ?></option>
                        <?php endforeach; ?>
                    </optgroup>
            ...
            <div>
                ...
                <textarea id="form_deskripsi" name="deskripsi" rows="3" required placeholder="Contoh: Penerimaan hasil penjualan bulan Mei..." class="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-white focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent transition"></textarea>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-white/10">
                ...
                <button type="submit" class="px-5 py-2.5 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition shadow-[0_0_15px_rgba(34,197,94,0.3)]">Simpan Kas Masuk</button>
            </div>
        </form>
    </div>
</div>
        ...
        <div class="p-6">
            ...
            <form method="post" action="<?= base_url('keuangan/kas-masuk-keluar/import-excel') ?>" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-white/10">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition shadow-[0_0_15px_rgba(34,197,94,0.3)]">Proses Import</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function openModal() {
        document.getElementById('modalTitle').innerHTML = '<i class="fas fa-arrow-down text-green-400 mr-2"></i>Tambah Kas Masuk';
    ...
    async function openEditModal(no_reff) {
        try {
            ...
            if (data.status === 'success') {
                document.getElementById('modalTitle').innerHTML = '<i class="fas fa-arrow-down text-green-400 mr-2"></i>Edit Kas Masuk';
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <p><span class="text-gray-500 inline-block w-32">Nominal</span>: <span class="text-green-400 font-bold font-mono">Rp ${parseInt(data.data.nominal).toLocaleString('id-ID')}</span></p>
