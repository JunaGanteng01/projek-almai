...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 print:hidden">
    <div>
        <h1 class="text-2xl font-bold text-white">Daftar Hutang</h1>
        <p class="text-gray-500 text-xs mt-1">Manajemen hutang kepada supplier yang belum lunas</p>
    </div>
    <div class="flex flex-wrap gap-2">
        ...
        <button onclick="window.print()" class="px-4 py-2 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition flex items-center gap-2 shadow-lg shadow-red-500/20">
...
<div id="filterSection" class="bg-[#111] border border-white/10 rounded-2xl p-6 mb-8 shadow-xl hidden animate-in fade-in slide-in-from-top-4 print:hidden">
    <form action="<?= base_url('keuangan/hutang') ?>" method="get" class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
        <div>
            <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Tanggal Mulai</label>
            <input type="date" name="start_date" value="<?= esc($startDate) ?>" class="w-full bg-black border border-white/10 rounded-xl px-4 py-3 focus:border-red-500 focus:outline-none text-white text-sm">
        </div>
        <div>
            <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Tanggal Akhir</label>
            <input type="date" name="end_date" value="<?= esc($endDate) ?>" class="w-full bg-black border border-white/10 rounded-xl px-4 py-3 focus:border-red-500 focus:outline-none text-white text-sm">
        </div>
        <button type="submit" class="w-full py-3 bg-red-500 hover:bg-red-600 text-white font-bold rounded-xl transition shadow-lg shadow-red-500/20">
            Tampilkan
        </button>
    </form>
</div>
...
<!-- Summary Card -->
<div class="bg-[#111] border border-red-500/20 rounded-2xl p-6 mb-8 shadow-xl print:bg-white print:text-black print:border-gray-400">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-red-500/10 rounded-2xl flex items-center justify-center border border-red-500/20 print:hidden">
                <i class="fas fa-file-invoice-dollar text-red-400 text-2xl"></i>
            </div>
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Total Hutang</p>
                <p class="text-3xl font-black text-red-400 print:text-black">Rp <?= number_format($totalHutang, 0, ',', '.') ?></p>
            </div>
        </div>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-white/5 border-b border-white/10 print:bg-gray-100 print:border-gray-400">
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest print:text-black">No. Faktur</th>
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest print:text-black">Supplier</th>
            ...
            <tbody class="divide-y divide-white/5 print:divide-gray-400">
                <?php if (!empty($hutangData)): ?>
                    <?php foreach ($hutangData as $row): ?>
                    <tr class="hover:bg-white/5 transition print:hover:bg-transparent">
                        <td class="px-6 py-4 text-sm font-mono text-gray-300 print:text-black"><?= esc($row['no_faktur'] ?? '-') ?></td>
                        <td class="px-6 py-4 text-sm text-gray-300 print:text-black"><?= esc($row['supplier_name'] ?? 'Unknown') ?></td>
                        ...
                        <td class="px-6 py-4 text-sm text-center hide-mobile print:hidden">
                            <a href="#" class="text-red-400 hover:text-red-300 transition font-bold text-xs">
...
<!-- Info Banner -->
<div class="mt-8 p-5 bg-red-500/5 border border-red-500/20 rounded-2xl flex items-start gap-4 print:hidden">
    ...
    <div>
        <p class="font-bold text-red-400 text-sm mb-1">Informasi Hutang</p>
        <ul class="text-xs text-gray-400 space-y-1 list-disc list-inside">
            <li>Hutang adalah kewajiban pembayaran kepada supplier yang belum dilunasi.</li>
            <li>Kelola hutang dengan baik untuk menjaga hubungan baik dengan supplier.</li>
            <li>Prioritaskan pembayaran hutang yang sudah melampaui jatuh tempo.</li>
        </ul>
    </div>
</div>
