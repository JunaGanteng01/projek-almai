...
<body class="bg-[#0b0b0b] text-gray-300 font-sans min-h-screen py-10 px-4 flex flex-col items-center">
    ...
    <div class="max-w-[595pt] w-full mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 no-print bg-[#111] border border-white/10 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            <a href="<?= base_url('keuangan/invoices') ?>" class="w-10 h-10 bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-center text-white transition">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h4 class="font-bold text-white text-sm">Invoice Customer</h4>
                <p class="text-xs text-gray-400 font-mono"><?= esc($invoice['invoice_number']) ?></p>
            </div>
        </div>
    ...
    <div class="w-full max-w-[595pt] overflow-x-auto pb-4 no-print scrollbar-thin">
        ...
        <div class="flex flex-col md:flex-row justify-between gap-6 pb-6 border-b border-white/10 pdf-border print-border">
            ...
            <div class="md:text-right space-y-1.5 print-text-black">
                <h1 class="text-4xl font-extrabold text-white tracking-tight font-title print-text-black">INVOICE</h1>
                <div class="space-y-0.5 font-mono text-xs text-gray-400 print-text-black">
                    <p class="font-bold text-sm pdf-accent-text print-text-accent">No: <?= esc($invoice['invoice_number']) ?></p>
                    <p>Tanggal: <?= date('d M Y', strtotime($invoice['tanggal'])) ?></p>
                    <?php if (!empty($invoice['jatuh_tempo'])): ?>
                        <p class="text-red-400 font-bold print-text-black">Jatuh Tempo: <?= date('d M Y', strtotime($invoice['jatuh_tempo'])) ?></p>
                    <?php endif; ?>
                    <p>Status: 
                        <?php if ($invoice['status'] === 'paid'): ?>
                            <span class="text-green-400 font-bold uppercase print-text-black">[ LUNAS / PAID ]</span>
                        <?php elseif ($invoice['status'] === 'cancelled'): ?>
                            <span class="text-red-400 font-bold uppercase print-text-black">[ DIBATALKAN ]</span>
                        <?php else: ?>
                            <span class="text-yellow-400 font-bold uppercase print-text-black">[ PENDING / BELUM BAYAR ]</span>
                        <?php endif; ?>
                    </p>
                </div>
            </div>
        ...
        <div class="py-2 print-text-black">
            ...
            <div class="space-y-0.5">
                <h3 class="text-base font-bold text-white print-text-black"><?= esc($invoice['customer_name']) ?></h3>
                <?php if ($invoice['customer_email']): ?>
                    <p class="text-xs text-gray-400 print-text-black"><i class="far fa-envelope mr-1.5 opacity-60"></i><?= esc($invoice['customer_email']) ?></p>
                <?php endif; ?>
                <?php if ($invoice['customer_phone']): ?>
                    <p class="text-xs text-gray-400 print-text-black"><i class="fas fa-phone mr-1.5 opacity-60"></i><?= esc($invoice['customer_phone']) ?></p>
                <?php endif; ?>
                <?php if (!empty($invoice['customer_address'])): ?>
                    <p class="text-xs text-gray-400 print-text-black mt-1 max-w-sm"><i class="fas fa-map-marker-alt mr-1.5 opacity-60"></i><?= nl2br(esc($invoice['customer_address'])) ?></p>
                <?php endif; ?>
            </div>
        </div>
                    ...
                    <?php foreach ($items as $idx => $item): ?>
                        <tr class="hover:bg-white/5 transition print:hover:bg-transparent">
                            ...
                            <td class="px-5 py-2 text-sm">
                                <p class="font-bold text-white print:text-black"><?= esc($item['name']) ?></p>
                            </td>
                            ...
                            <td class="px-5 py-2 text-right font-mono text-gray-400 print-text-black">
                                <?= ($invoice['currency'] ?? 'IDR') === 'USD' ? '$ ' . number_format($item['price'], 2, '.', ',') : 'Rp ' . number_format($item['price'], 0, ',', '.') ?>
                            </td>
                            <td class="px-5 py-2 text-right font-bold text-white font-mono print:text-black">
                                <?= ($invoice['currency'] ?? 'IDR') === 'USD' ? '$ ' . number_format($item['total'], 2, '.', ',') : 'Rp ' . number_format($item['total'], 0, ',', '.') ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <div class="space-y-4">
            ...
            <?php if ($invoice['notes']): ?>
                <div class="text-xs text-gray-400 space-y-1 print-text-black">
                    ...
                    <p class="italic leading-relaxed print-text-black"><?= nl2br(esc($invoice['notes'])) ?></p>
                </div>
            <?php endif; ?>
                ...
                <div class="bg-white/5 border border-white/10 rounded-xl p-5 space-y-3.5 pdf-border flex flex-col justify-between h-full print-border print:bg-transparent">
                    <div class="space-y-3.5">
                        <div class="flex justify-between text-xs text-gray-400 font-medium print-text-black">
                            <span>Subtotal:</span>
                            <span class="font-mono"><?= ($invoice['currency'] ?? 'IDR') === 'USD' ? '$ ' . number_format($invoice['subtotal'], 2, '.', ',') : 'Rp ' . number_format($invoice['subtotal'], 0, ',', '.') ?></span>
                        </div>
                        <div class="flex justify-between text-xs text-gray-400 font-medium print-text-black">
                            <span>PPN (<?= number_format($invoice['ppn'], 0) ?>%):</span>
                            <span class="font-mono"><?= ($invoice['currency'] ?? 'IDR') === 'USD' ? '$ ' . number_format($invoice['ppn_nominal'], 2, '.', ',') : 'Rp ' . number_format($invoice['ppn_nominal'], 0, ',', '.') ?></span>
                        </div>
                    </div>
                    <div class="border-t border-white/10 pdf-border pt-2.5 flex justify-between items-center print-border">
                        ...
                        <span class="text-lg font-black pdf-accent-text font-mono print-text-accent"><?= ($invoice['currency'] ?? 'IDR') === 'USD' ? '$ ' . number_format($invoice['total'], 2, '.', ',') : 'Rp ' . number_format($invoice['total'], 0, ',', '.') ?></span>
                    </div>
                </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 pt-6 border-t border-white/10 text-center pdf-border print-border">
            <div>
                ...
                <div class="text-[10px] text-gray-400 font-mono leading-tight bg-white/5 p-2 rounded-lg border border-white/5 inline-block text-left min-w-[200px] print-digital-stamp">
                    ...
                    <span class="text-gray-500 print-text-gray">Name:</span> <?= esc(substr($invoice['customer_name'], 0, 20)) ?><br>
                    <span class="text-gray-500 print-text-gray">Date:</span> <?= date('Y-m-d H:i') ?> WITA
                </div>
                <p class="text-xs font-semibold text-gray-300 mt-2 print:text-black"><?= esc($invoice['customer_name']) ?></p>
            </div>
            <div>
                ...
                <div class="text-[10px] text-gray-400 font-mono leading-tight bg-white/5 p-2 rounded-lg border border-white/5 inline-block text-left min-w-[200px] print-digital-stamp">
                    ...
                    <span class="text-gray-500 print-text-gray">Ref:</span> ALMAI-SEC-<?= strtoupper(substr(md5($invoice['invoice_number']), 0, 8)) ?><br>
                    <span class="text-gray-500 print-text-gray">Date:</span> <?= date('Y-m-d H:i', strtotime($invoice['created_at'] ?: 'now')) ?> WITA
                </div>
                <p class="text-xs font-semibold text-white mt-2 print:text-black">Susani Halim</p>
            </div>
        </div>
    ...
    <script>
        ...
        function downloadPDF() {
            ...
            const opt = {
                margin:       0,
                filename:     'Invoice-' + '<?= str_replace('/', '_', esc($invoice['invoice_number'])) ?>' + '.pdf',
