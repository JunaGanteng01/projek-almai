...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 print:hidden">
    <div>
        ...
        <p class="text-gray-500 text-xs mt-1">Manajemen pembelian dari data CSV</p>
    </div>
    <div class="flex flex-wrap gap-2">
        ...
        <a href="<?= base_url('keuangan/pembelian/form') ?>" class="px-4 py-2 bg-accent text-black font-bold rounded-xl hover:bg-white transition flex items-center gap-2 shadow-lg shadow-accent/20">
            <i class="fas fa-plus"></i> Tambah
        </a>
        <a href="<?= base_url('keuangan/pembelian/export-csv') ?>" class="px-4 py-2 bg-green-500/10 text-green-400 font-bold rounded-xl hover:bg-green-500/20 transition border border-green-500/30 flex items-center gap-2">
...
<div id="filterSection" class="bg-[#111] border border-white/10 rounded-2xl p-6 mb-8 shadow-xl hidden animate-in fade-in slide-in-from-top-4 print:hidden">
    <form action="<?= base_url('keuangan/pembelian') ?>" method="get" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
        <div>
            <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Cari</label>
            <input type="text" name="search" value="<?= esc($search) ?>" placeholder="Nomor PI, supplier..." class="w-full bg-black border border-white/10 rounded-xl px-4 py-3 focus:border-accent focus:outline-none text-white text-sm">
        </div>
        <div>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
    <div class="bg-[#111] border border-red-500/20 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-black text-red-400"><?= isset($summary['total_transaksi']) ? $summary['total_transaksi'] : 0 ?></p>
            </div>
        </div>
    </div>
    <div class="bg-[#111] border border-orange-500/20 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-orange-500/10 rounded-xl flex items-center justify-center border border-orange-500/20">
                <i class="fas fa-coins text-orange-400 text-lg"></i>
            </div>
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest">Total Nilai</p>
                <p class="text-xl font-black text-orange-400">Rp <?= isset($summary['total_nilai']) ? number_format($summary['total_nilai'], 0, ',', '.') : '0' ?></p>
            </div>
        </div>
    </div>
    <div class="bg-[#111] border border-amber-500/20 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-black text-amber-400">Rp <?= isset($summary['total_lunas']) ? number_format($summary['total_lunas'], 0, ',', '.') : '0' ?></p>
            </div>
        </div>
    </div>
    <div class="bg-[#111] border border-rose-500/20 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-rose-500/10 rounded-xl flex items-center justify-center border border-rose-500/20">
                <i class="fas fa-hourglass text-rose-400 text-lg"></i>
            </div>
            <div>
                <p class="text-xs font-bold text-gray-500 uppercase tracking-widest">Hutang</p>
                <p class="text-xl font-black text-rose-400">Rp <?= number_format($hutang_total, 0, ',', '.') ?></p>
            </div>
        </div>
    </div>
</div>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-white/5 border-b border-white/10 print:bg-gray-100 print:border-gray-400">
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest print:text-black">No. PI</th>
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-widest print:text-black">Nama Supplier</th>
            ...
            <tbody class="divide-y divide-white/5 print:divide-gray-400">
                <?php if (!empty($pembelian)): ?>
                    ...
                    <tr class="hover:bg-white/5 transition print:hover:bg-transparent">
                        <td class="px-6 py-4 text-sm font-mono text-gray-300 print:text-black"><?= esc($row['nomor_pi']) ?></td>
                        <td class="px-6 py-4 text-sm text-gray-300 print:text-black"><?= esc($row['nama_supplier']) ?></td>
                        ...
                        <td class="px-6 py-4 text-sm text-center hide-mobile print:hidden">
                            <div class="flex justify-center gap-2">
                                <a href="<?= base_url('keuangan/pembelian/detail/' . $row['id']) ?>" class="text-blue-400 hover:text-blue-300 transition font-bold text-xs" title="Lihat">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="<?= base_url('keuangan/pembelian/form/' . $row['id']) ?>" class="text-amber-400 hover:text-amber-300 transition font-bold text-xs" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form action="<?= base_url('keuangan/pembelian/delete/' . $row['id']) ?>" method="post" style="display:inline;" onsubmit="return confirm('Yakin hapus?');">
                                    <?= csrf_field() ?>
...
<?php if ($pager): ?>
<div class="mt-6">
    <?= $pager->links('pembelian', 'tailwind_full') ?>
</div>
<?php endif; ?>
...
<dialog id="importModal" class="modal">
    <div class="modal-box bg-[#111] border border-white/10">
        <h3 class="font-bold text-lg text-white mb-4">Import Data Pembelian dari CSV</h3>
        <form action="<?= base_url('keuangan/pembelian/import-csv') ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <div class="mb-4">
                ...
                <p class="text-xs text-gray-500 mt-2">Format: CSV dengan kolom: Nomor PI, Nama Supplier, Email, Alamat, dst.</p>
            </div>
