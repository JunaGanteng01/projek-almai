...
<head>
    ...
    <title>Pembelian - <?= esc($pembelian['nomor_pi']) ?></title>
    <!-- Fonts -->
...
<body class="bg-[#0b0b0b] text-gray-300 font-sans min-h-screen py-10 px-4 flex flex-col items-center">
    ...
    <div class="max-w-[595pt] w-full mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 no-print bg-[#111] border border-white/10 rounded-2xl p-4 shadow-xl">
        <div class="flex items-center gap-3">
            <a href="<?= base_url('keuangan/pembelian') ?>" class="w-10 h-10 bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-center text-white transition">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h4 class="font-bold text-white text-sm">Pembelian / PO</h4>
                <p class="text-xs text-gray-400 font-mono"><?= esc($pembelian['nomor_pi']) ?></p>
            </div>
        </div>
    ...
    <div class="w-full max-w-[595pt] overflow-x-auto pb-4 no-print scrollbar-thin">
        ...
        <div class="flex flex-col md:flex-row justify-between gap-6 pb-6 border-b border-white/10 pdf-border print-border">
            ...
            <div class="md:text-right space-y-1.5 print-text-black">
                <h1 class="text-4xl font-extrabold text-white tracking-tight font-title print-text-black">PEMBELIAN</h1>
                <div class="space-y-0.5 font-mono text-xs text-gray-400 print-text-black">
                    <p class="font-bold text-sm pdf-accent-text print-text-accent">No: <?= esc($pembelian['nomor_pi']) ?></p>
                    <p>Tanggal: <?= date('d M Y', strtotime($pembelian['tanggal_transaksi'])) ?></p>
                    <?php if (!empty($pembelian['tanggal_jatuh_tempo'])): ?>
                        <p class="text-red-400 font-bold print-text-black">Jatuh Tempo: <?= date('d M Y', strtotime($pembelian['tanggal_jatuh_tempo'])) ?></p>
                    <?php endif; ?>
                    <p>Status:
        ...
        <div class="py-2 print-text-black">
            <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 print-text-gray">Dibeli Dari (Bill From):</p>
            <div class="space-y-0.5">
                <h3 class="text-base font-bold text-white print-text-black"><?= esc($pembelian['nama_supplier']) ?></h3>
                <?php if ($pembelian['email']): ?>
                    <p class="text-xs text-gray-400 print-text-black"><i class="far fa-envelope mr-1.5 opacity-60"></i><?= esc($pembelian['email']) ?></p>
                <?php endif; ?>
                <?php if ($pembelian['alamat']): ?>
                    <p class="text-xs text-gray-400 print-text-black"><i class="fas fa-map-marker-alt mr-1.5 opacity-60"></i><?= esc($pembelian['alamat']) ?></p>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <?php if ($pembelian['catatan']): ?>
                <div class="text-xs text-gray-400 space-y-1 print-text-black">
                    ...
                    <p class="italic leading-relaxed print-text-black"><?= nl2br(esc($pembelian['catatan'])) ?></p>
                </div>
            <?php endif; ?>
                ...
                <div class="bg-white/5 border border-white/10 rounded-xl p-5 space-y-3.5 pdf-border flex flex-col justify-between h-full print-border print:bg-transparent">
                    <div class="space-y-3.5">
                        <div class="flex justify-between text-xs text-gray-400 font-medium print-text-black">
                            <span>Subtotal:</span>
                            <span class="font-mono">Rp <?= number_format($pembelian['subtotal'] ?? 0, 0, ',', '.') ?></span>
                        </div>
                        <div class="flex justify-between text-xs text-gray-400 font-medium print-text-black">
                            <span>PPN (<?= number_format($pembelian['ppn'], 0) ?>%):</span>
                            <span class="font-mono">Rp <?= number_format(($pembelian['subtotal'] ?? 0) * ($pembelian['ppn'] / 100), 0, ',', '.') ?></span>
                        </div>
                    </div>
                    <div class="border-t border-white/10 pdf-border pt-2.5 flex justify-between items-center print-border">
                        ...
                        <span class="text-lg font-black pdf-accent-text font-mono print-text-accent">Rp <?= number_format($pembelian['total'] ?? 0, 0, ',', '.') ?></span>
                    </div>
                </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 pt-6 border-t border-white/10 text-center pdf-border print-border">
            <div>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider mb-1 print-text-gray">Penerima (Kami)</p>
                <div class="text-[10px] text-gray-400 font-mono leading-tight bg-white/5 p-2 rounded-lg border border-white/5 inline-block text-left min-w-[200px] print-digital-stamp">
                    ...
                    <span class="text-gray-500 print-text-gray">Name:</span> PT. Alma Indonesia Raya<br>
                    <span class="text-gray-500 print-text-gray">Date:</span> <?= date('Y-m-d H:i') ?> WITA
                </div>
                <p class="text-xs font-semibold text-gray-300 mt-2 print:text-black">PT. Alma Indonesia Raya</p>
            </div>
            <div>
                <p class="text-[10px] text-gray-500 uppercase tracking-wider mb-1 print-text-gray">Supplier (<?= esc(substr($pembelian['nama_supplier'], 0, 20)) ?>)</p>
                <div class="text-[10px] text-gray-400 font-mono leading-tight bg-white/5 p-2 rounded-lg border border-white/5 inline-block text-left min-w-[200px] print-digital-stamp">
                    <span class="text-accent font-bold print-text-accent"><i class="fas fa-shield-alt mr-1"></i> Secured by System</span><br>
                    <span class="text-gray-500 print-text-gray">Ref:</span> ALMAI-SEC-<?= strtoupper(substr(md5($pembelian['nomor_pi']), 0, 8)) ?><br>
                    <span class="text-gray-500 print-text-gray">Date:</span> <?= date('Y-m-d H:i', strtotime($pembelian['created_at'] ?: 'now')) ?> WITA
                </div>
    ...
    <script>
        ...
        function downloadPDF() {
            ...
            const opt = {
                margin:       0,
                filename:     'Pembelian-' + '<?= str_replace('/', '_', esc($pembelian['nomor_pi'])) ?>' + '.pdf',
