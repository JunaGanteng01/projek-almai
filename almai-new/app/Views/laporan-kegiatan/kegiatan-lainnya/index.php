...
<div class="w-full pb-12">
    ...
    <div class="mb-6 flex flex-col md:flex-row justify-between md:items-center gap-4 px-6 md:px-0">
        <h1 class="text-xl md:text-2xl font-black text-white uppercase tracking-tight">Kegiatan Lainnya</h1>
    </div>
    ...
    <div class="bg-[#111] border border-white/10 rounded-2xl p-4 shadow-xl mb-6">
        <form method="GET" action="<?= base_url('laporan-kegiatan/kegiatan-lainnya') ?>" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                <?php if(!empty($search)): ?>
                    <a href="<?= base_url('laporan-kegiatan/kegiatan-lainnya') ?>" class="bg-red-500/20 hover:bg-red-500/30 text-red-500 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition flex items-center justify-center">
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                    <tr class="bg-black/50 text-accent text-center font-bold text-[10px] uppercase tracking-wider">
                        ...
                        <th class="p-3 border-b border-r border-white/10">Nama & Jenis Kegiatan</th>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-white/[0.02] text-center transition-colors">
                            ...
                            <td class="p-3 border-r border-white/5 text-left text-white font-bold">
                                <?= esc($k['name']) ?: '-' ?><br>
                                <span class="text-[10px] text-gray-400 font-normal"><?= esc($k['subcategory']) ?></span>
                            </td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($k['students']) ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs">-</td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($k['category']) ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($k['wpa_name']) ?: '-' ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs text-left"><?= esc($k['location']) ?: '-' ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs whitespace-nowrap">
                                <?php if (!empty($k['is_recurring'])): ?>
                                    Setiap <?= esc($k['recurring_day']) ?><br>
                                    <span class="text-[10px] opacity-75"><?= esc($k['recurring_time']) ?></span>
                                <?php else: ?>
                                    <?= (!empty($k['event_date']) && $k['event_date'] != '0000-00-00 00:00:00') ? date('d M Y, H:i', strtotime($k['event_date'])) : '-' ?>
                                <?php endif; ?>
                            </td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs text-left">
                                <?php
                                    $desc = strip_tags($k['description'] ?? '');
                                    echo esc(strlen($desc) > 50 ? substr($desc, 0, 50) . '...' : ($desc ?: '-'));
                                ?>
                            </td>
                            <td class="p-3 flex items-center justify-center gap-2">
                                <a href="<?= base_url('superadmin/layanan/edit/'.($k['table_name'] === 'layanan_artikel' ? 'artikel' : strtolower($k['subcategory'] ?? 'event')).'/'.$k['id']) ?>" class="text-blue-400 hover:text-blue-300 transition-colors" title="Lihat di Superadmin">
