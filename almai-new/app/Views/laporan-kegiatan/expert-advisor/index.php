...
<div class="w-full pb-12">
    ...
    <div class="mb-6 flex flex-col md:flex-row justify-between md:items-center gap-4 px-6 md:px-0">
        <h1 class="text-xl md:text-2xl font-black text-white uppercase tracking-tight">Expert Advisor</h1>
    </div>
    ...
    <div class="bg-[#111] border border-white/10 rounded-2xl p-4 shadow-xl mb-6">
        <form method="GET" action="<?= base_url('laporan-kegiatan/expert-advisor') ?>" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
            ...
            <div class="flex gap-2">
                ...
                <?php if(!empty($search)): ?>
                    <a href="<?= base_url('laporan-kegiatan/expert-advisor') ?>" class="bg-red-500/20 hover:bg-red-500/30 text-red-500 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition flex items-center justify-center">
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                    <tr class="bg-black/50 text-accent text-center font-bold text-[10px] uppercase tracking-wider">
                        ...
                        <th class="p-3 border-b border-r border-white/10">Nama Layanan</th>
                        <th class="p-3 border-b border-r border-white/10">Penjelasan Layanan</th>
                        <th class="p-3 border-b border-r border-white/10">Jml Klien</th>
                        <th class="p-3 border-b border-r border-white/10">Produk</th>
                        <th class="p-3 border-b border-r border-white/10">Nama WPA</th>
                        <th class="p-3 border-b border-r border-white/10">Winning Rate</th>
                ...
                <tbody class="divide-y divide-white/5">
                    <?php if(empty($expertAdvisors)): ?>
                        <tr>
                            <td colspan="10" class="p-6 text-center text-gray-500 italic">Belum ada layanan untuk modul ini dari Superadmin.</td>
                        </tr>
                    <?php else: ?>
                        <?php 
                        ...
                        <tr class="hover:bg-white/[0.02] text-center transition-colors">
                            ...
                            <td class="p-3 border-r border-white/5 text-left text-white font-bold"><?= esc($ea['name']) ?: '-' ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs text-left">
                                <?php
                                    $desc = strip_tags($ea['subcategory'] ?? '');
                                    echo esc(strlen($desc) > 50 ? substr($desc, 0, 50) . '...' : ($desc ?: '-'));
                                ?>
                            </td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($ea['students']) ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($ea['category']) ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($ea['wpa_name']) ?: '-' ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs"><?= esc($ea['winning_rate'] ?? '-') ?></td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs whitespace-nowrap">
                                <?php if (!empty($ea['is_recurring'])): ?>
                                    Setiap <?= esc($ea['recurring_day']) ?><br>
                                    <span class="text-[10px] opacity-75"><?= esc($ea['recurring_time']) ?></span>
                                <?php else: ?>
                                    <?= (!empty($ea['event_date']) && $ea['event_date'] != '0000-00-00 00:00:00') ? date('d M Y, H:i', strtotime($ea['event_date'])) : '-' ?>
                                <?php endif; ?>
                            </td>
                            <td class="p-3 border-r border-white/5 text-gray-400 text-xs">
                                <?php
                                    $ket = strip_tags($ea['description'] ?? '');
                                    echo esc(strlen($ket) > 30 ? substr($ket, 0, 30) . '...' : ($ket ?: '-'));
                                ?>
                            </td>
                            <td class="p-3 flex items-center justify-center gap-2">
                                <a href="<?= base_url('superadmin/layanan/edit/'.($ea['table_name'] === 'layanan_artikel' ? 'artikel' : strtolower($ea['subcategory'] ?? 'event')).'/'.$ea['id']) ?>" class="text-blue-400 hover:text-blue-300 transition-colors" title="Lihat di Superadmin">
