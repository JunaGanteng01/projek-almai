...
<body class="antialiased">
    ...
    <div class="flex flex-col lg:flex-row min-h-screen relative overflow-hidden">
        ...
        <div class="lg:hidden fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a] border-b border-white/10 px-4 py-3 flex justify-between items-center">
            <a href="<?= base_url('laporan-kegiatan/advokasi') ?>" class="flex items-center gap-2 text-gray-400 hover:text-white">
        ...
        <aside id="sidebar" class="w-full lg:w-80 bg-[#0a0a0a] border-r border-white/10 fixed lg:relative h-full z-40 transform -translate-x-full lg:translate-x-0 transition-transform overflow-y-auto custom-scrollbar">
            <div class="p-4 border-b border-white/10 sticky top-0 bg-[#0a0a0a] z-10">
                <a href="<?= base_url('laporan-kegiatan/advokasi') ?>" class="hidden lg:flex items-center gap-2 text-gray-400 hover:text-white mb-4">
                    <i class="fas fa-arrow-left"></i>
                    <span class="text-sm">Kembali ke Advokasi</span>
                </a>
                <h2 class="font-bold text-lg leading-tight" id="kelasTitle">Kelas Advokasi Trading (Basic)</h2>
                <div class="flex items-center gap-2 mt-2">
                    ...
                    <span class="text-xs text-gray-400">Tim Legal ALMAI</span>
                </div>
            </div>
        ...
        <main class="flex-1 pt-14 lg:pt-0 relative">
            ...
            <div class="h-64 md:h-80 w-full relative overflow-hidden">
                <img id="headerBanner" src="" class="absolute inset-0 w-full h-full object-cover">
                <div class="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/40 to-transparent"></div>
                <div class="absolute bottom-10 left-6 right-6 md:left-12 md:right-12">
                    <h2 class="text-accent font-black uppercase tracking-[0.3em] text-[10px] mb-2" id="headerCategory">Academic Module</h2>
                    <h1 class="text-3xl md:text-5xl font-black text-white" id="headerTitle">Memuat Materi...</h1>
                </div>
            </div>
            ...
            <div class="relative min-h-[600px]">
                ...
                <div class="max-w-4xl mx-auto p-6 md:p-12 <?= !$isPaid ? 'blur-md grayscale' : '' ?>">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 border-b border-white/5 pb-8">
                        <div>
                            <span class="inline-block px-3 py-1 bg-accent/10 border border-accent/20 text-accent text-[10px] font-black uppercase tracking-widest rounded-full mb-3" id="currentModuleNumber">Modul 1</span>
                            <h1 class="text-2xl md:text-3xl font-black mt-1" id="currentModuleTitle">Memuat Materi...</h1>
                        </div>
                        <div class="flex gap-3">
                            <button onclick="markComplete()" id="completeBtn" class="flex-1 md:flex-none px-8 py-4 bg-accent text-black font-black uppercase tracking-widest rounded-2xl hover:bg-white transition-all text-xs shadow-lg">
                                <i class="fas fa-check mr-2"></i> Tandai Selesai
                            </button>
                            <button onclick="nextModule()" class="flex-1 md:flex-none px-8 py-4 border border-white/10 text-white font-black uppercase tracking-widest rounded-2xl hover:border-accent hover:text-accent transition-all text-xs">
                    ...
                    <div class="flex gap-8 border-b border-white/5 mb-10">
                        <button onclick="showTab('materi')" class="tab-btn active pb-4 border-b-2 border-accent text-accent font-black uppercase tracking-widest text-[10px]" data-tab="materi">Isi Materi</button>
                        <button onclick="showTab('resources')" class="tab-btn pb-4 border-b-2 border-transparent text-gray-500 font-black uppercase tracking-widest text-[10px] hover:text-white transition-all" data-tab="resources">E-Book & Lampiran</button>
                        <button onclick="showTab('catatan')" class="tab-btn pb-4 border-b-2 border-transparent text-gray-500 font-black uppercase tracking-widest text-[10px] hover:text-white transition-all" data-tab="catatan">Catatan Riset</button>
                    </div>
                    ...
                    <div id="tab-materi" class="tab-content">
                        ...
                        <div class="mt-8 p-10 bg-white/5 rounded-[2.5rem] border border-white/5 overflow-hidden relative">
                            ...
                            <h4 class="text-white font-black uppercase tracking-widest text-xs mb-6 flex items-center gap-3">
                                <i class="fas fa-star text-accent"></i> Key Learnings
                            </h4>
    ...
    <script>
        ...
        const modules = [
            { 
                id: 1, 
                title: 'SENIN — Protect: Legalitas & Keamanan | Aman', 
                duration: '45 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=2070&auto=format&fit=crop', 
                image: 'https://images.unsplash.com/photo-1563986768609-322da13575f3?q=80&w=2070&auto=format&fit=crop',
                handbook: 'Trading bukan hanya aktivitas finansial, tetapi juga aktivitas yang memiliki aspek hukum dan perlindungan. Banyak trader mengalami kerugian bukan karena strategi yang salah, melainkan karena berada di lingkungan trading yang tidak aman dan tidak terproteksi secara legal.',
                topics: [
                    {
                    ...
                    {
                        title: 'Risiko Penipuan & Investasi Bodong',
                        content: `<h2>Risiko Penipuan & Investasi Bodong</h2><p>Investasi bodong seringkali menggunakan kedok trading untuk melakukan penipuan. Waspadai tawaran "Passive Income" tanpa risiko, karena trading pada dasarnya memiliki risiko fluktuasi harga yang nyata.</p>`
                    },
                    {
                        title: 'Keamanan Akun & Data Trading',
                        content: `<h2>Keamanan Akun & Data Trading</h2><p>Gunakan pengamanan berlapis seperti Two-Factor Authentication (2FA), kelola password secara berkala, dan hindari login di perangkat publik untuk mencegah pencurian akses dan manipulasi akun.</p>`
                    },
                    {
                        title: 'Peran Penasihat Berjangka (WPA)',
                        content: `<h2>Peran Penasihat Berjangka (WPA)</h2><p>Penasihat Berjangka atau Wealth Protection Advisor (WPA) berperan mendampingi trader agar tetap berada di jalur yang aman, membimbing dalam pemilihan platform, serta memberikan edukasi mitigasi risiko secara profesional.</p>`
                    },
                    {
                        title: 'Mekanisme Penyelesaian Sengketa',
                        content: `<h2>Mekanisme Penyelesaian Sengketa</h2><p>Jika terjadi perselisihan dengan broker, trader memiliki jalur resmi melalui pembuatan aduan ke Bappebti, mediasi di Bursa, hingga arbitrase di BAKTI (Badan Arbitrase Perdagangan Berjangka Komoditi).</p>`
                    }
                ],
                highlights: [
                    ...
                    'Menyadari peran pendampingan Penasihat Berjangka'
                ]
            },
            { 
                id: 2, 
                title: 'SELASA — Understand: Struktur Market | Paham', 
                duration: '40 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1611974715853-2b8ef9a36ce8?q=80&w=2070&auto=format&fit=crop',
                image: 'https://images.unsplash.com/photo-1642104704074-907c0698cbd9?q=80&w=1932&auto=format&fit=crop',
                handbook: 'Pergerakan market bukan terjadi secara acak, melainkan terbentuk dari interaksi antara pelaku pasar, aliran likuiditas, serta mekanisme transaksi yang terstruktur.',
                topics: [
                    {
                        title: 'Cara Kerja Market (Forex, Crypto, Komo)',
                        content: `<h2>Cara Kerja Market</h2><p>Market digerakkan oleh hukum permintaan and penawaran. Peserta pasar mulai dari bank sentral, institusi keuangan, hingga retail trader bersaing untuk mendapatkan harga terbaik.</p>`
                    },
                    {
                        title: 'Konsep Supply & Demand',
                        content: `<h2>Konsep Supply & Demand</h2><p>Harga akan naik jika permintaan melebihi penawaran, dan turun jika sebaliknya. Mengidentifikasi area di mana ketidakseimbangan ini terjadi adalah kunci menemukan peluang trading.</p>`
                    },
                    {
                        title: 'Market Maker vs ECN',
                        content: `<h2>Market Maker vs ECN</h2><p>Market Maker (Bandar) bertindak sebagai lawan transaksi Anda, sementara ECN (Electronic Communication Network) langsung menghubungkan Anda ke penyedia likuiditas global.</p>`
                    },
                    {
                        title: 'Sesi Trading Global',
                        content: `<h2>Sesi Trading Global</h2><p>Pahami karakteristik Sesi Asia (tenang), Sesi London (mulai ramai), dan Sesi New York (volatilitas tinggi) untuk menentukan waktu trading paling efektif.</p>`
                    }
                ],
                highlights: [
                    'Memahami mekanisme dasar pergerakan harga',
                    'Mengenali perbedaan karakter broker MM & ECN',
                    'Menentukan waktu trading berdasarkan sesi global'
                ]
            },
            { 
                id: 3, 
                title: 'RABU — Control: Risk & Mindset | Rasional', 
                duration: '50 Menit', 
                completed: false,
                ...
                handbook: 'Keberhasilan dalam trading tidak hanya ditentukan oleh strategi, tetapi sangat dipengaruhi oleh kemampuan dalam mengelola risiko dan mengendalikan emosi.',
                topics: [
                    {
                    ...
                    {
                        title: 'Position Sizing & Lot Calculation',
                        content: `<h2>Position Sizing</h2><p>Ukuran lot harus disesuaikan dengan jarak Stop Loss dan batas risiko uang Anda. Jangan gunakan lot asal besar (Overlot) jika ingin bertahan lama di market.</p>`
                    },
                    {
                        title: 'Psikologi Trading (Fear & Greed)',
                        content: `<h2>Psikologi Trading</h2><p>Emosi takut kehilangan uang dan serakah ingin cepat kaya adalah musuh terbesar trader. Kestabilan mental lebih penting daripada keunggulan teknikal.</p>`
                    },
                    {
                        title: 'Disiplin dan Konsistensi',
                        content: `<h2>Disiplin dan Konsistensi</h2><p>Patuhi rencana trading yang telah dibuat (Trading Plan). Kedisiplinan adalah pembeda antara trader profesional dan spekulan amatir.</p>`
                    }
                ],
                highlights: [
            ...
            { 
                id: 4, 
                title: 'KAMIS — Audit: Evaluasi Sistem | Teruji', 
                duration: '40 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1454165833767-0274b24f48a8?q=80&w=2070&auto=format&fit=crop',
                image: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2026&auto=format&fit=crop',
                handbook: 'Trading yang dilakukan tanpa evaluasi berisiko besar mengulang kesalahan yang sama secara terus-menerus.',
                topics: [
                    {
                        title: 'Backtest & Forward Test',
                        content: `<h2>Pengujian Strategi</h2><p>Uji strategi Anda menggunakan data masa lalu (Backtest) dan akun demo (Forward Test) sebelum menggunakan uang sungguhan untuk melihat probabilitas keberhasilannya.</p>`
                    },
                    {
                        title: 'Penggunaan Trading Journal',
                        content: `<h2>Trading Journal</h2><p>Catat setiap entry, exit, alasan, and perasaan Anda saat trading. Jurnal adalah guru terbaik yang menunjukkan di mana letak kesalahan objektif Anda.</p>`
                    },
                    {
                        title: 'Analisis Winrate & Risk Reward',
                        content: `<h2>Metrik Performa</h2><p>Kualitas trading dinilai dari rasio Risk-Reward (minimal 1:2) dan Winrate. Anda tidak perlu menang setiap saat untuk menjadi profitabel secara akumulatif.</p>`
                    }
                ],
                highlights: [
            ...
            { 
                id: 5, 
                title: 'JUMAT — Optimize: Strategi & Konsistensi | Stabil', 
                duration: '45 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1542744173-8e7e548d5d17?q=80&w=2070&auto=format&fit=crop',
                image: 'https://images.unsplash.com/photo-1551288049-bbbda536687a?q=80&w=2070&auto=format&fit=crop',
                handbook: 'Strategi trading yang efektif bukanlah strategi yang terus berubah, melainkan strategi yang disempurnakan dan dijalankan secara konsisten.',
                topics: [
                    {
                        title: 'Penyempurnaan Strategi Trading',
                        content: `<h2>Penyempurnaan Strategi</h2><p>Fokuslah menguasai satu atau dua metode hingga ahli. Hindari "Holy Grail Hunting" atau kebiasaan bergonta-ganti indikator tanpa tujuan jelas.</p>`
                    },
                    {
                        title: 'Integrasi Teknologi (EA/Trading Tools)',
                        content: `<h2>Integrasi Teknologi</h2><p>Manfaatkan Expert Advisor (EA) atau alat bantu lainnya untuk membantu eksekusi yang lebih cepat dan disiplin tanpa gangguan emosi manusia.</p>`
                    },
                    {
                        title: 'Scaling dan Compounding',
                        content: `<h2>Growing the Account</h2><p>Kembangkan modal Anda secara organik melalui kekuatan bunga bergulung (Compounding) daripada melakukan deposit besar dengan risiko tinggi.</p>`
                    }
                ],
                highlights: [
                    'Pengembangan strategi yang stabil',
                    'Pemanfaatan alat bantu teknologi trading',
                    'Manajemen pertumbuhan akun berkelanjutan'
                ]
            },
            { 
                id: 6, 
                title: 'SABTU — Defend: Proteksi & Advokasi | Terlindungi', 
                duration: '50 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?q=80&w=2070&auto=format&fit=crop', 
                image: 'https://images.unsplash.com/photo-1505664194779-8beaceb93744?q=80&w=2070&auto=format&fit=crop',
                handbook: 'Dalam aktivitas trading, risiko tidak hanya berasal dari market, tetapi juga dari potensi permasalahan dengan pihak ketiga seperti broker.',
                topics: [
                    {
                        title: 'Hak Trader & Perlindungan Hukum',
                        content: `<h2>Hak Anda sebagai Trader</h2><p>Anda berhak atas transparansi transaksi, keamanan dana, and layanan yang adil sesuai regulasi. Memahami hak ini membuat Anda tidak mudah ditekan oleh broker.</p>`
                    },
                    {
                        title: 'Prosedur Pelaporan & Mediasi',
                        content: `<h2>Langkah Menghadapi Masalah</h2><p>Jika WD ditahan atau harga dimanipulasi, lakukan dokumentasi lengkap dan segera laporkan melalui alur resmi Bappebti atau lembaga advokasi terkait.</p>`
                    },
                    {
                        title: 'Pentingnya Dokumentasi Transaksi',
                        content: `<h2>Bukti adalah Senjata</h2><p>Selalu simpan screenshot bukti transaksi, riwayat chat, and log aktivitas platform. Dokumentasi ini adalah syarat mutlak dalam memenangkan sengketa.</p>`
                    }
                ],
                highlights: [
                    'Pemahaman hak-hak dasar trader',
                    'Alur pelaporan sengketa yang taktis',
                    'Pentingnya manajemen bukti dan dokumentasi'
                ]
            },
            { 
                id: 7, 
                title: 'MINGGU — Recap: Evaluasi & Outlook | Siap', 
                duration: '35 Menit', 
                completed: false,
                banner: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2026&auto=format&fit=crop',
                image: 'https://images.unsplash.com/photo-1507679799987-c73779587ccf?q=80&w=2071&auto=format&fit=crop',
                handbook: 'Trader yang profesional tidak hanya fokus pada aktivitas trading harian, tetapi juga melakukan evaluasi dan perencanaan secara berkala.',
                topics: [
                    {
                        title: 'Review Mingguan & Trading Plan',
                        content: `<h2>Recap & Planning</h2><p>Gunakan hasil audit hari Kamis untuk merancang rencana trading minggu depan. Sesuaikan target profit and risiko dengan kondisi market terkini.</p>`
                    },
                    {
                        title: 'Outlook Market Mingguan',
                        content: `<h2>Membaca Peluang Kedepan</h2><p>Analisis kalender ekonomi dan tren di timeframe besar untuk mendapatkan gambaran arah market yang lebih jernih sebelum Senin dimulai.</p>`
                    },
                    {
                        title: 'Penetapan Goal Trading Realistis',
                        content: `<h2>Focusing on Growth</h2><p>Tetapkan target yang realistis. Trading adalah maraton jangka panjang, bukan sprint yang mengharuskan Anda kaya dalam semalam.</p>`
                    }
                ],
                highlights: [
                    ...
                    topicsHtml = `
                        <div class="mt-4 ml-8 space-y-2 border-l border-white/5 pl-4">
                            ${module.topics.map((topic, tIndex) => `
                                <div onclick="selectTopic(${tIndex}, event)" class="py-2 px-3 rounded-lg cursor-pointer transition-all hover:bg-white/5 ${tIndex === currentTopicIndex ? 'text-accent font-bold bg-accent/5' : 'text-gray-500 text-xs'} flex items-center gap-2">
        ...
        function updateContent() {
            ...
            document.getElementById('headerTitle').textContent = module.title;
            document.getElementById('headerCategory').textContent = `Framework: ${['AMAN', 'PAHAM', 'RASIONAL', 'TERUJI', 'STABIL', 'TERLINDUNGI', 'SIAP'][currentModuleIndex]}`;
            ...
            if (isPaid) {
                ...
                if (module.image && currentTopicIndex === 0) {
        ...
        function nextModule() {
            if (currentTopicIndex < modules[currentModuleIndex].topics.length - 1) {
                ...
                Swal.fire({
                    ...
                    text: 'Anda telah menyelesaikan seluruh framework Advokasi Almai: Aman → Paham → Rasional → Teruji → Stabil → Terlindungi → Siap',
                    icon: 'success',
                    background: '#0a0a0a',
                    color: '#ffffff',
