...
<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 mb-6">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold"><?= $totalDirect ?? 0 ?></p>
...
<div class="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6">
    <form action="<?= base_url('superadmin/users') ?>" method="get" class="flex flex-row flex-wrap items-center gap-2 w-full lg:w-auto flex-1">
    ...
    <?php if ($canWrite ?? false): ?>
        <div class="flex flex-wrap gap-2 w-full sm:w-auto">
            <button type="button" onclick="submitMergeReferral()" class="px-6 py-2 bg-purple-500 text-white font-bold rounded-xl hover:bg-white hover:text-black transition text-center whitespace-nowrap">
                <i class="fas fa-object-group mr-2"></i> Gabung Referral
            </button>
            <button onclick="checkAffiliatorAccess()" class="px-6 py-2 bg-blue-500 text-white font-bold rounded-xl hover:bg-white hover:text-black transition text-center whitespace-nowrap">
                <i class="fas fa-link mr-2"></i> Ubah Affiliator
            </button>
            <a href="<?= base_url('superadmin/users/export-excel') . (!empty($_SERVER['QUERY_STRING']) ? '?' . $_SERVER['QUERY_STRING'] : '') ?>" class="px-6 py-2 bg-green-600 text-white font-bold rounded-xl hover:bg-white hover:text-black transition text-center whitespace-nowrap">
                <i class="fas fa-file-excel mr-2"></i> Export Excel
            </a>
            <a href="<?= base_url('superadmin/users/create') ?>" class="px-6 py-2 bg-accent text-black font-bold rounded-xl hover:bg-white transition text-center whitespace-nowrap">
...
<form id="mergeReferralForm" action="<?= base_url('superadmin/users/merge-referrals') ?>" method="post">
<?= csrf_field() ?>
<!-- Users Table -->
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead class="bg-black/50">
                <tr>
                    <th class="text-center px-3 py-3 w-10"><input type="checkbox" id="selectAllMerge" onchange="toggleAllMerge(this)"></th>
                ...
                <?php else: ?>
                    ...
                    ?>
                        <tr class="border-t border-white/5 hover:bg-white/5">
                            <td class="px-3 py-3 text-center"><input type="checkbox" name="user_ids[]" value="<?= $user['id'] ?>" class="merge-checkbox"></td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="<?= base_url('superadmin/users/view/' . $user['id']) ?>" class="w-8 h-8 flex items-center justify-center bg-accent/20 text-accent rounded-lg hover:bg-accent hover:text-black transition" title="Lihat">
                                        <i class="fas fa-eye text-xs"></i>
                                    </a>
                                    <?php if ($canWrite ?? false): ?>
                                        <a href="<?= base_url('superadmin/users/edit/' . $user['id']) ?>" class="w-8 h-8 flex items-center justify-center bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500 hover:text-white transition" title="Edit">
                                            <i class="fas fa-edit text-xs"></i>
                                        </a>
                                        <a href="<?= base_url('superadmin/users/toggle-status/' . $user['id']) ?>" class="w-8 h-8 flex items-center justify-center bg-yellow-500/20 text-yellow-400 rounded-lg hover:bg-yellow-500 hover:text-black transition" title="Toggle Status">
...
<script>
    function checkAffiliatorAccess() {
        Swal.fire({
            title: 'Keamanan Superadmin',
            text: 'Masukkan password untuk mengakses fitur ini:',
            input: 'password',
            inputAttributes: {
                autocapitalize: 'off',
                autocorrect: 'off'
            ...
            if (result.isConfirmed) {
                window.location.href = '<?= base_url('superadmin/users/batch-affiliator') ?>';
            }
        })
    }

    function toggleAllMerge(source) {
        checkboxes = document.getElementsByClassName('merge-checkbox');
        for (var i = 0; i < checkboxes.length; i++) {
            checkboxes[i].checked = source.checked;
        }
    }

    function submitMergeReferral() {
        const checkboxes = document.getElementsByClassName('merge-checkbox');
        let count = 0;
        ...
        if (count < 2) {
            Swal.fire({
                title: 'Pilih User',
                text: 'Pilih minimal 2 user untuk digabungkan referralnya.',
                icon: 'warning',
                background: '#111',
                color: '#fff',
                confirmButtonColor: '#33E818'
        ...
        Swal.fire({
            title: 'Gabungkan Referral?',
            html: `Anda akan menggabungkan referral dari <strong>${count}</strong> user.<br><span class="text-sm text-gray-400">Total downline mereka akan saling dijumlahkan.</span>`,
            icon: 'question',
            background: '#111',
            color: '#fff',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                document.getElementById('mergeReferralForm').submit();
            }
        })
    }
    ...
    function confirmDeleteIndex(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = '<?= base_url('superadmin/users/delete') ?>/' + id;
                form.submit();
            }
        })
    }
