...
<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 rounded-2xl p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <p class="text-gray-400 text-sm">Saldo Poin Superadmin</p>
                <p class="text-3xl font-bold text-yellow-400"><?= number_format($poinBalance) ?> <span class="text-lg">Almai Poin</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-[#111] border border-white/10 rounded-2xl p-6">
            ...
            <form action="<?= base_url('superadmin/poin/share/store') ?>" method="post">
                <?= csrf_field() ?>
                ...
                <!-- Bank Info -->
                <div class="mb-4">
                    <label class="block text-sm text-gray-400 mb-2">Rekening Tujuan Transfer <span class="text-red-400">*</span></label>
                    <div class="space-y-3">
                        <input type="text" name="bank_name" required placeholder="Nama Bank / E-Wallet" value="<?= esc($userData['bank_name'] ?? '') ?>"
                            class="w-full px-4 py-3 bg-black border border-white/20 rounded-xl focus:outline-none focus:border-accent">
                        <input type="text" name="bank_account_name" required placeholder="Nama Pemilik Rekening" value="<?= esc($userData['account_name'] ?? '') ?>"
                            class="w-full px-4 py-3 bg-black border border-white/20 rounded-xl focus:outline-none focus:border-accent">
                        <input type="text" name="bank_account_number" required placeholder="Nomor Rekening" value="<?= esc($userData['account_number'] ?? '') ?>"
                            class="w-full px-4 py-3 bg-black border border-white/20 rounded-xl focus:outline-none focus:border-accent">
                        <p class="text-xs text-gray-500 mt-1">Anda bisa mengubah detail rekening untuk setiap link yang dibuat.</p>
                    </div>
                </div>
                        ...
                        <?php foreach ($pendingSharings as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-white/10">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <form action="<?= base_url('superadmin/poin/share/delete/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
                    ...
                    <div class="space-y-3">
                        <?php foreach ($waitingVerification as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-orange-500/30">
                                ...
                                <div class="flex gap-2">
                                    <form action="<?= base_url('superadmin/poin/share/verify/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
...
<script>
    ...
    function showRejectModal(id) {
        document.getElementById('rejectForm').action = '<?= base_url('superadmin/poin/share/reject/') ?>' + id;
