...
<header class="bg-[#080808]/95 backdrop-blur-md border-b border-white/[0.08] px-4 sm:px-6 lg:px-8 h-16 sm:h-[68px] flex justify-between items-center fixed top-0 right-0 left-0 md:left-64 z-40 transition-all duration-300">
    ...
    <div class="flex items-center gap-2.5 sm:gap-3">
        ...
        <div class="relative">
            ...
            <div id="notifMenu" class="hidden absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 bg-[#0e0e0e] border border-white/[0.1] rounded-2xl shadow-2xl overflow-hidden z-50 animate-fadeIn">
                <div class="p-3.5 border-b border-white/[0.08] flex justify-between items-center bg-black/40">
                    ...
                    <?php if ($unreadCount > 0): ?>
                    <a href="<?= base_url('superadmin/notifications/read-all') ?>" class="text-[11px] font-semibold text-accent hover:underline">Tandai semua dibaca</a>
                    <?php endif; ?>
                </div>
                <div class="max-h-80 overflow-y-auto custom-scrollbar divide-y divide-white/[0.04]">
                    ...
                    <a href="<?= base_url('superadmin/notifications/read/' . $notif['id']) ?>" class="block px-4 py-3 hover:bg-white/[0.03] transition <?= $notif['is_read'] ? 'opacity-50' : '' ?>">
                ...
                <a href="<?= base_url('superadmin/notifications') ?>" class="block p-3 text-center text-xs font-bold text-accent hover:text-white hover:bg-accent/10 border-t border-white/[0.08] transition">
        ...
        <div class="relative">
            ...
            <!-- Profile Dropdown -->
            <div id="profileMenu" class="hidden absolute right-0 top-full mt-2 w-52 bg-[#0e0e0e] border border-white/[0.1] rounded-2xl shadow-2xl overflow-hidden z-50 animate-fadeIn">
                ...
                <div class="p-1.5 space-y-0.5 text-xs">
                    <a href="<?= base_url('superadmin/setting') ?>" class="flex items-center gap-2.5 px-3 py-2 text-gray-300 hover:text-white hover:bg-white/[0.06] rounded-xl transition">
                        <i class="fas fa-cog w-4 text-gray-400"></i> Settings
                    </a>
                    <a href="<?= base_url('superadmin/logout') ?>" class="w-full flex items-center gap-2.5 px-3 py-2 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition">
...
document.addEventListener('click', function(e) {
    ...
    const notifButton = e.target.closest('button[onclick="toggleNotifMenu()"]');
    
    if (!profileButton && profileMenu && !profileMenu.contains(e.target)) {
        profileMenu.classList.add('hidden');
    }
    if (!notifButton && notifMenu && !notifMenu.contains(e.target)) {
