...
$pageSubtitle = 'Kelola pengurus yang ditampilkan di halaman WPA dan About';
?>
...
<div class="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6">
    <!-- Search Form -->
    <form action="<?= base_url('superadmin/tim') ?>" method="get" class="flex flex-row flex-wrap items-center gap-2 w-full lg:w-auto flex-1">
        <div class="flex gap-2 w-full sm:w-auto sm:flex-1 lg:flex-none">
            ...
            <?php if (!empty($search)): ?>
                <a href="<?= base_url('superadmin/tim') ?>" class="px-4 py-2 bg-red-500/20 text-red-400 border border-red-500/50 rounded-xl hover:bg-red-500 hover:text-white transition flex items-center justify-center">
    ...
    <a href="<?= base_url('superadmin/tim/create') ?>" class="w-full sm:w-auto px-6 py-2 bg-accent text-black font-bold rounded-xl hover:bg-white transition text-center whitespace-nowrap">
                        ...
                        <tr class="border-t border-white/5 hover:bg-white/5 transition">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="<?= base_url('superadmin/tim/edit/' . $member['id']) ?>" class="w-8 h-8 flex items-center justify-center bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500 hover:text-white transition" title="Edit">
                                        <i class="fas fa-edit text-xs"></i>
                                    </a>
                                    <button type="button" data-name="<?= esc($member['name'], 'attr') ?>" onclick="confirmDelete(<?= (int) $member['id'] ?>, this.dataset.name)" class="w-8 h-8 flex items-center justify-center bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition" title="Hapus">
                                        <i class="fas fa-trash text-xs"></i>
                                    </button>
                                </div>
                                <form id="deleteForm-<?= $member['id'] ?>" action="<?= base_url('superadmin/tim/delete/' . $member['id']) ?>" method="post" class="hidden"><?= csrf_field() ?></form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
