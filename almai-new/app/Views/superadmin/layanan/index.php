...
<div class="flex flex-col gap-4 mb-6">
    ...
    <div class="flex items-center gap-2 overflow-x-auto scrollbar-hide -mx-4 px-4">
        <a href="<?= base_url('superadmin/layanan') ?>" class="px-4 py-2 rounded-full text-[11px] md:text-sm font-bold whitespace-nowrap transition <?= empty($currentKategori) ? 'bg-accent text-black' : 'bg-white/5 text-gray-400 hover:bg-white/10' ?>">SEMUA</a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <?php if ($canWrite ?? false): ?>
            <a href="<?= base_url('superadmin/layanan/create') ?>" class="w-full flex items-center justify-center gap-2 px-6 py-3 bg-accent text-black rounded-xl font-bold text-sm hover:bg-white transition shadow-lg shadow-accent/10">
                        ...
                        <tr class="hover:bg-white/5 transition group">
                            ...
                            <td class="px-4 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <?php if ($canWrite ?? false): ?>
                                        <a href="<?= base_url('superadmin/layanan/edit/' . $layanan['layanan_type'] . '/' . $layanan['id']) ?>" class="w-10 h-10 flex items-center justify-center bg-white/5 text-gray-400 rounded-xl hover:bg-white/10 hover:text-white transition shadow-sm" title="Edit">
                                            <i class="fas fa-pencil-alt text-xs"></i>
                                        </a>
                                        <form action="<?= base_url('superadmin/layanan/toggle-status/' . $layanan['layanan_type'] . '/' . $layanan['id']) ?>" method="POST" class="inline">
                                            <?= csrf_field() ?>
                                        ...
                                        <button type="button" onclick="showDeleteModal('<?= base_url('superadmin/layanan/delete/' . $layanan['layanan_type'] . '/' . $layanan['id']) ?>', '<?= esc($layanan['name'] ?? $layanan['title'] ?? 'Layanan') ?>')" class="w-10 h-10 flex items-center justify-center bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition shadow-sm" title="Hapus">
