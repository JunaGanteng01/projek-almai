...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        <a href="<?= base_url('superadmin/absensi') ?>" class="p-2 hover:bg-white/10 rounded-lg">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl p-6">
        <form action="<?= base_url('superadmin/absensi/store') ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <select name="registration_audience" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3">
                        ...
                        <option value="cwpa_wpa">Khusus CWPA dan WPA</option>
                    </select>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <textarea name="keterangan" rows="3" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3" placeholder="Keterangan opsional..."></textarea>
                </div>
            </div>

            <div class="grid grid-cols-1 gap-6">
                <div>
                    <label class="block text-sm text-gray-400 mb-2">Gambar Banner / Poster (Opsional)</label>
                    <input type="file" name="banner_image" accept=".jpg,.jpeg,.png" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3 text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-accent/10 file:text-accent hover:file:bg-accent/20">
                    <p class="text-xs text-gray-500 mt-2">Format: JPG, JPEG, PNG. Maksimal 10MB.</p>
                </div>
            </div>
                    ...
                    <select name="format_notif_wa" class="w-full bg-[#0a0a0a] border border-white/10 rounded-lg px-4 py-3 text-sm">
                        ...
                        <?php if (isset($whatsapp_templates)) : ?>
                            <?php foreach ($whatsapp_templates as $tpl) : ?>
                                <option value="<?= $tpl['id'] ?>"><?= esc($tpl['nama_template']) ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </select>
