...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        <a href="<?= base_url('superadmin/absensi') ?>" class="w-10 h-10 bg-white/5 hover:bg-white/10 rounded-xl flex items-center justify-center transition">
    ...
    <?php if (!empty($kegiatan['banner_image'])): ?>
    <div class="bg-[#111] border border-white/10 rounded-xl overflow-hidden mb-6 p-6">
        <h3 class="font-bold text-lg mb-4">Banner / Poster</h3>
        <img src="<?= base_url($kegiatan['banner_image']) ?>" alt="Banner" class="w-full max-w-2xl rounded-lg border border-white/10">
    </div>
    <?php endif; ?>
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
        <div class="p-6 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="<?= base_url('superadmin/absensi/export-csv/' . urlencode($kegiatan['kegiatan_type_id'] ?? $kegiatanType) . '/' . $kegiatan['id']) ?>" class="px-4 py-2 bg-green-500/20 text-green-400 font-medium rounded-lg hover:bg-green-500/30 transition text-sm flex items-center gap-2">
                    <i class="fas fa-file-csv"></i> Export CSV
                </a>
                <a href="<?= base_url('superadmin/absensi/export-xlsx/' . urlencode($kegiatan['kegiatan_type_id'] ?? $kegiatanType) . '/' . $kegiatan['id']) ?>" class="px-4 py-2 bg-blue-500/20 text-blue-400 font-medium rounded-lg hover:bg-blue-500/30 transition text-sm flex items-center gap-2">
                    <i class="fas fa-file-excel"></i> Export XLSX
                </a>
                <a href="<?= base_url('superadmin/absensi/export-pdf/' . urlencode($kegiatan['kegiatan_type_id'] ?? $kegiatanType) . '/' . $kegiatan['id']) ?>" target="_blank" class="px-4 py-2 bg-red-500/20 text-red-400 font-medium rounded-lg hover:bg-red-500/30 transition text-sm flex items-center gap-2">
                        ...
                        <?php $no = 1; foreach ($peserta as $p): ?>
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="<?= base_url('superadmin/absensi/delete-peserta/' . $p['id']) ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus peserta ini dari absensi?')" class="p-2 inline-flex items-center justify-center text-red-400 hover:bg-red-400/10 rounded transition" title="Hapus">
