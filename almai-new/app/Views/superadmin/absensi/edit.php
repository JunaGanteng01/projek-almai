...
<div class="space-y-6">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl p-6">
        <form action="<?= base_url('superadmin/absensi/update/' . urlencode($kegiatanType) . '/' . $kegiatan['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <?php if (!empty($kegiatan['banner_image'])): ?>
                        <div class="mb-3">
                            <img src="<?= base_url($kegiatan['banner_image']) ?>" alt="Banner" class="w-full max-w-md rounded-lg border border-white/10">
                        </div>
                    <?php endif; ?>
                    ...
                    <p class="text-xs text-gray-500 mt-2">Format: JPG, JPEG, PNG. Maksimal 10MB. Biarkan kosong jika tidak ingin mengubah.</p>
                </div>
            </div>
