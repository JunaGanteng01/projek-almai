...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    ...
    <a href="<?= base_url('superadmin/merchandise') ?>" class="px-4 py-2 border border-white/20 rounded-xl hover:border-accent hover:text-accent transition text-sm">
...
<div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
    <a href="<?= base_url('superadmin/merchandise/redemptions') ?>" class="bg-[#111] border <?= !$filter || $filter === 'all' ? 'border-accent' : 'border-white/10' ?> rounded-xl p-4 hover:border-accent/50 transition">
        <p class="text-2xl font-bold"><?= $stats['total'] ?></p>
        <p class="text-xs text-gray-500">Total</p>
    </a>
    <a href="<?= base_url('superadmin/merchandise/redemptions?status=pending') ?>" class="bg-[#111] border <?= $filter === 'pending' ? 'border-yellow-500' : 'border-white/10' ?> rounded-xl p-4 hover:border-yellow-500/50 transition">
        <p class="text-2xl font-bold text-yellow-500"><?= $stats['pending'] ?></p>
        <p class="text-xs text-gray-500">Pending</p>
    </a>
    <a href="<?= base_url('superadmin/merchandise/redemptions?status=processing') ?>" class="bg-[#111] border <?= $filter === 'processing' ? 'border-blue-500' : 'border-white/10' ?> rounded-xl p-4 hover:border-blue-500/50 transition">
        <p class="text-2xl font-bold text-blue-500"><?= $stats['processing'] ?></p>
        <p class="text-xs text-gray-500">Diproses</p>
    </a>
    <a href="<?= base_url('superadmin/merchandise/redemptions?status=shipped') ?>" class="bg-[#111] border <?= $filter === 'shipped' ? 'border-purple-500' : 'border-white/10' ?> rounded-xl p-4 hover:border-purple-500/50 transition">
        <p class="text-2xl font-bold text-purple-500"><?= $stats['shipped'] ?></p>
        <p class="text-xs text-gray-500">Dikirim</p>
    </a>
    <a href="<?= base_url('superadmin/merchandise/redemptions?status=completed') ?>" class="bg-[#111] border <?= $filter === 'completed' ? 'border-accent' : 'border-white/10' ?> rounded-xl p-4 hover:border-accent/50 transition">
...
function openUpdateModal(item) {
    document.getElementById('updateForm').action = '<?= base_url('superadmin/merchandise/redemptions/update/') ?>' + item.id;
