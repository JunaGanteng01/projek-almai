...
<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
    ...
    <div class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
        <a href="<?= base_url('superadmin/merchandise/redemptions') ?>" class="w-full sm:w-auto px-4 py-2 border border-white/20 rounded-xl hover:border-accent hover:text-accent transition text-sm text-center">
            <i class="fas fa-exchange-alt mr-2"></i> Lihat Penukaran
        </a>
        <a href="<?= base_url('superadmin/merchandise/create') ?>" class="w-full sm:w-auto px-4 py-2 bg-accent text-black font-bold rounded-xl hover:bg-white transition text-sm text-center">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div class="col-span-full bg-[#111] border border-white/10 rounded-xl p-12 text-center">
        ...
        <a href="<?= base_url('superadmin/merchandise/create') ?>" class="inline-block mt-4 px-4 py-2 bg-accent text-black font-bold rounded-xl text-sm">Tambah Merchandise</a>
    </div>
    <?php else: ?>
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl overflow-hidden hover:border-accent/50 transition">
        ...
        <div class="p-4">
            ...
            <div class="flex gap-2">
                <a href="<?= base_url('superadmin/merchandise/edit/' . $item['id']) ?>" class="flex-1 py-2 border border-white/20 rounded-lg text-center text-sm hover:border-accent hover:text-accent transition">
                    <i class="fas fa-edit mr-1"></i> Edit
                </a>
                <form action="<?= base_url('superadmin/merchandise/delete/' . $item['id']) ?>" method="post" class="flex-1" onsubmit="return confirm('Hapus merchandise ini?')">
                    <?= csrf_field() ?>
