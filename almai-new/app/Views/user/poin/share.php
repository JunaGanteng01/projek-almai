...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="<?= base_url('user/poin') ?>" class="px-4 py-2 bg-white/10 rounded-xl hover:bg-white/20 transition">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-[#111] border border-white/10 rounded-2xl p-6">
            ...
            <form action="<?= base_url('user/poin/share/store') ?>" method="post">
                <?= csrf_field() ?>
                    ...
                    <div class="space-y-3">
                        <?php if(empty($userData['bank_name']) || empty($userData['account_name']) || empty($userData['account_number'])): ?>
                            <div class="p-3 bg-red-500/20 text-red-400 border border-red-500/30 rounded-lg text-sm mb-3">
                                <i class="fas fa-exclamation-triangle mr-2"></i> Data rekening Anda belum lengkap. Silakan lengkapi di menu Pengaturan.
                            </div>
                        <?php endif; ?>
                        ...
                        <?php foreach ($pendingSharings as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-white/10">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <form action="<?= base_url('user/poin/share/delete/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
                    ...
                    <div class="space-y-3">
                        <?php foreach ($waitingVerification as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-orange-500/30">
                                ...
                                <div class="flex gap-2">
                                    <form action="<?= base_url('user/poin/share/verify/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
...
<script>
    ...
    function showRejectModal(id) {
        document.getElementById('rejectForm').action = '<?= base_url('user/poin/share/reject/') ?>' + id;
