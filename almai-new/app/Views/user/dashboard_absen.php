...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="<?= base_url('user/dashboard') ?>" class="flex items-center gap-1.5 bg-[#111] hover:bg-[#1a1a1a] text-gray-300 hover:text-white border border-white/10 rounded-lg px-3 py-1.5 transition text-xs font-medium">
...
window.onload = function() {
   // Re-evaluate button states on page load (e.g., after back navigation)
   document.querySelectorAll('.checkin-btn-wrapper').forEach(wrapper => {
       const isCheckedIn = wrapper.dataset.isCheckedIn === '1';
       const checkinBtn = wrapper.querySelector('.checkin-button');
       const shareBtn = wrapper.querySelector('.share-button');

       if (isCheckedIn) {
           if (checkinBtn) {
               checkinBtn.innerHTML = '<i class="fas fa-check-circle"></i> Sudah Check-in';
               checkinBtn.classList.remove('bg-accent', 'hover:bg-accent/90', 'text-black');
               checkinBtn.classList.add('bg-gray-700', 'text-gray-400', 'cursor-not-allowed');
               checkinBtn.removeAttribute('href');
               checkinBtn.setAttribute('disabled', 'true');
           }
           if (shareBtn) {
               shareBtn.innerHTML = '<i class="fas fa-share-alt"></i> Bagikan';
               shareBtn.classList.remove('bg-blue-500/10', 'text-blue-400', 'hover:bg-blue-500', 'hover:text-white', 'border-blue-500/20');
               shareBtn.classList.add('bg-gray-800', 'text-gray-500', 'cursor-not-allowed');
