...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="<?= base_url('admin-admin-wpa/absensi/create') ?>" class="px-6 py-3 bg-accent text-black font-bold rounded-xl hover:bg-white transition flex items-center justify-center gap-2">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
        <div class="p-6 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <form action="<?= base_url('admin-admin-wpa/absensi') ?>" method="GET" class="flex flex-col md:flex-row gap-3">
                ...
                <?php if(!empty($filters['q']) || !empty($filters['tipe'])): ?>
                    <a href="<?= base_url('admin-admin-wpa/absensi') ?>" class="px-4 py-2 bg-white/5 text-white font-medium rounded-lg hover:bg-white/10 transition text-sm">Reset</a>
                <?php endif; ?>
            </form>
        </div>
                            ...
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="<?= base_url('admin-admin-wpa/absensi/detail/' . $item['kegiatan_type_id'] . '/' . $item['id']) ?>" class="p-2 text-green-400 hover:bg-green-400/10 rounded transition flex items-center gap-2" title="Detail">
                                        ...
                                        <a href="<?= base_url('admin-admin-wpa/absensi/delete/' . urlencode($item['tipe']) . '/' . $item['id']) ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data absensi ini?')" class="p-2 text-red-400 hover:bg-red-400/10 rounded transition flex items-center gap-2" title="Hapus">
