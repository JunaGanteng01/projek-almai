...
<div class="w-full">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h1 class="text-3xl font-black text-white tracking-tight uppercase">Tambah Transaksi Manual</h1>
            <p class="text-gray-400 mt-1 uppercase text-xs tracking-widest font-bold">Input transaksi untuk WPA yang Anda kelola</p>
        </div>
        <a href="<?= base_url('admin-wpa/transaksi') ?>" class="px-5 py-2.5 bg-[#111] border border-white/10 rounded-xl text-gray-400 hover:text-white transition flex items-center gap-2">
    ...
    <!-- Form Area -->
    <form action="<?= base_url('admin-wpa/transaksi/store-manual') ?>" method="POST" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-[#111] border border-white/10 rounded-2xl p-6 shadow-xl">
                    <label class="block text-[10px] uppercase tracking-widest font-black text-accent mb-4">Pilih User (Dalam Jaringan Anda)</label>
                ...
                <!-- Select Service -->
                <div class="bg-[#111] border border-white/10 rounded-2xl p-6 shadow-xl">
                    <label class="block text-[10px] uppercase tracking-widest font-black text-accent mb-4">Pilih Layanan / Produk (Milik WPA Anda)</label>
                    <div class="relative group">
                        <select name="service_full" id="service_select" class="w-full px-4 py-3.5 bg-black border border-white/10 rounded-xl focus:border-accent focus:outline-none transition group-hover:border-white/20 select2-dark" required onchange="updatePrice(this)">
                            ...
                            <?php foreach ($services as $s): 
                                $durationInfo = '';
                                if ($s['type'] === 'subscription' && !empty($s['duration_days'])) {
                                    $durationInfo = ' ('. $s['duration_days'] . ' Days)';
                                }
                                $wpaSuffix = !empty($s['wpa_name']) ? ' - ' . esc($s['wpa_name']) : '';
                            ?>
                                <option value="<?= $s['type'] ?>:<?= $s['id'] ?>" data-price="<?= $s['price'] ?>" <?= old('service_full') == ($s['type'].':'.$s['id']) ? 'selected' : '' ?>>
                                    [<?= strtoupper($s['type']) ?>] <?= esc($s['title']) ?><?= $durationInfo ?> - Rp <?= number_format($s['price'], 0, ',', '.') ?><?= $wpaSuffix ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <p class="mt-2 text-[10px] text-gray-500 italic">* Hanya layanan yang dikelola oleh WPA Anda yang muncul</p>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <!-- Amount -->
                <div class="bg-[#111] border border-white/10 rounded-2xl p-6 shadow-xl">
                    <label class="block text-[10px] uppercase tracking-widest font-black text-accent mb-4">Nominal Transaksi (Rp)</label>
                    <div class="relative group">
                ...
                <!-- Notes -->
                <div class="bg-[#111] border border-white/10 rounded-2xl p-6 shadow-xl">
                    <label class="block text-[10px] uppercase tracking-widest font-black text-accent mb-4">Catatan / Deskripsi (Opsional)</label>
