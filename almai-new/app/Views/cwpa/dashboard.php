...
<div class="mb-4">
    ...
    <h1 class="text-lg font-bold text-white"><?= esc(session()->get('cwpaName') ?? session()->get('userName')) ?></h1>
</div>
...
<div class="relative w-full bg-gradient-to-br from-[#0a3d1f] via-[#0c4a24] to-[#0a3d1f] rounded-2xl p-4 mb-4 overflow-hidden shadow-xl border border-[#1a5c32]">
    ...
    <div class="relative z-10 mb-4">
        <div class="flex items-center gap-2 mb-1">
            <span class="text-gray-300 text-xs">Jumlah Saldo</span>
    ...
    <div class="relative z-10 grid grid-cols-3 md:flex md:flex-wrap gap-2 mb-4">
        <a href="<?= base_url('cwpa/dashboard/withdraw') ?>" class="flex items-center justify-center gap-1 bg-accent hover:bg-accent/90 text-black font-bold text-[11px] sm:text-xs py-2.5 md:px-4 rounded-lg transition shadow-lg">
            <i class="fas fa-arrow-up text-sm"></i>
            <span>Tarik Dana</span>
        </a>
        <button onclick="doCheckin()" id="checkinBtn" <?= !$canCheckin ? 'disabled' : '' ?> class="flex items-center justify-center gap-1 <?= $canCheckin ? 'bg-white/10 hover:bg-white/20' : 'bg-white/5 cursor-not-allowed opacity-50' ?> text-white font-bold text-[11px] sm:text-xs py-2.5 md:px-4 rounded-lg transition border border-white/20">
            <i class="fas fa-check-circle text-sm"></i>
            <span>Checkin</span>
        </button>
        <a href="<?= base_url('cwpa/dashboard/absen') ?>" class="relative flex items-center justify-center gap-1 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 font-bold text-[11px] sm:text-xs py-2.5 md:px-4 rounded-lg transition border border-blue-500/30">
    ...
    <!-- Almai Poin Section -->
    <a href="<?= base_url('cwpa/dashboard/poin') ?>" class="relative z-10 flex items-center justify-between bg-black/20 hover:bg-black/30 backdrop-blur-sm rounded-xl p-3 transition border border-white/10 group">
...
<div class="grid grid-cols-3 gap-3 mb-6">
    <!-- Sistem Referral -->
    <a href="<?= base_url('cwpa/dashboard/referral') ?>" class="bg-[#111] hover:bg-[#1a1a1a] border border-white/10 rounded-xl p-4 transition group flex flex-col items-center text-center">
    ...
    <!-- Tukar Merchandise -->
    <a href="<?= base_url('cwpa/dashboard/poin/merchandise') ?>" class="bg-[#111] hover:bg-[#1a1a1a] border border-white/10 rounded-xl p-4 transition group flex flex-col items-center text-center">
        <div class="w-14 h-14 bg-accent/20 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition">
    ...
    <!-- Bagi Point -->
    <a href="<?= base_url('cwpa/dashboard/poin/share') ?>" class="bg-[#111] hover:bg-[#1a1a1a] border border-white/10 rounded-xl p-4 transition group flex flex-col items-center text-center">
        <div class="w-14 h-14 bg-accent/20 rounded-xl flex items-center justify-center mb-3 group-hover:scale-110 transition">
...
<div class="mb-6">
    ...
    <div class="grid grid-cols-2 gap-3 mb-4">
        ...
        <div class="bg-[#111] border border-white/10 rounded-xl p-3">
            ...
            <p class="text-2xl font-bold text-white mb-1"><?= number_format($stats['totalSertifikat'] ?? 0) ?></p>
    ...
    <div class="grid grid-cols-2 gap-3">
        ...
        <div class="bg-[#111] border border-white/10 rounded-xl p-3">
            ...
            <p class="text-2xl font-bold text-white mb-1"><?= number_format($stats['totalTransaksi'] ?? 0) ?></p>
        ...
        <div class="bg-[#111] border border-white/10 rounded-xl p-3">
            ...
            <p class="text-2xl font-bold text-white mb-1"><?= count($upcomingEvents ?? []) ?></p>
...
<!-- Portfolio Saya Card -->
<a href="<?= base_url('cwpa/dashboard/portofolio') ?>" class="block bg-gradient-to-br from-[#0a3d1f] to-[#0c4a24] rounded-xl p-4 mb-6 border border-[#1a5c32] hover:border-accent/50 transition group relative overflow-hidden">
    <!-- Background Image -->
...
<div class="mb-24">
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <h3 class="text-lg font-bold text-white">Event & Jadwal</h3>
            <p class="text-xs text-gray-400">Daftar event dan jadwal rutin di platform Almai</p>
        </div>
        <a href="<?= base_url('cwpa/dashboard/layanan') ?>" class="text-accent text-xs hover:underline">Lihat Semua</a>
    </div>

    <!-- Tab Buttons -->
    <div class="flex border-b border-white/10 mb-6 relative">
        <button onclick="switchTab('calendar')" id="tab-calendar" class="flex-1 pb-3 text-sm font-medium border-b-2 border-accent text-accent transition-all duration-300">
            Kalender & Jadwal
        </button>
        <button onclick="switchTab('list')" id="tab-list" class="flex-1 pb-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-300 transition-all duration-300">
            Event Mendatang
        </button>
    </div>
    ...
    <div id="content-calendar" class="transition-all duration-300">
        <div class="bg-[#121212] border border-white/5 rounded-xl p-5">
            <div class="flex items-center justify-center gap-6 mb-6">
                <button onclick="changeMonth(-1)" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white transition">
                    <i class="fas fa-chevron-left text-xs"></i>
                </button>
                <h4 class="font-bold text-base text-white tracking-wide uppercase" id="calendarMonth"><?= date('F Y') ?></h4>
                <button onclick="changeMonth(1)" class="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white transition">
            ...
            <div class="grid grid-cols-7 gap-1 mb-4">
                <?php foreach (['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'] as $day): ?>
                    <div class="text-center text-[10px] uppercase tracking-wider text-gray-500 font-medium"><?= $day ?></div>
                <?php endforeach; ?>
            </div>
    ...
    <div id="content-list" class="hidden transition-all duration-300">
        ...
        for ($i = 0; $i < $lookAheadDays; $i++) {
            ...
            foreach ($allEvents ?? [] as $event) {
                $isMatch = false;
                $displayTime = !empty($event['recurring_time']) ? $event['recurring_time'] : date('H:i', strtotime($event['event_date']));
                ...
                elseif ($event['is_recurring']) {
                    if ($event['recurring_frequency'] === 'daily') {
                        $isMatch = true;
                    } elseif ($event['recurring_frequency'] === 'weekly' && $event['recurring_day'] === $dayName) {
                        $isMatch = true;
                    } elseif ($event['recurring_frequency'] === 'monthly' && (int)$event['recurring_day'] === $dayOfMonth) {
                        $isMatch = true;
                    }
                }
        ...
        <?php if (empty($eventsByDate)): ?>
            <div class="text-center py-12 bg-[#121212] rounded-xl border border-white/5">
                ...
                <?php foreach ($eventsByDate as $dateKey => $group): ?>
                    <div class="overflow-hidden rounded-xl border border-white/10 bg-[#121212]" data-date="<?= $dateKey ?>">
                        <div class="bg-accent px-4 py-2 flex justify-between items-center">
                            <span class="text-black font-bold text-sm"><?= $group['day_name'] ?></span>
                            <span class="text-black font-medium text-xs"><?= $group['date_formatted'] ?></span>
                        </div>
                        <div class="divide-y divide-white/5">
                            <?php foreach ($group['events'] as $event): ?>
                                <div class="p-4 hover:bg-white/5 transition group">
                                    <div class="flex gap-4">
                                        <div class="w-24 shrink-0 text-[10px] text-gray-400 pt-0.5">
                                            <?= $event['display_time'] ?> WIB
                                        </div>
                                        <div class="flex-1">
                                            <div class="flex items-start gap-2 mb-1">
                                                <div class="w-1.5 h-1.5 rounded-full bg-accent mt-1.5 shrink-0"></div>
                                                <h4 class="text-sm font-bold text-white leading-tight transition">
                                                    <?= esc($event['title']) ?>
                                                </h4>
                                            </div>
                                            <?php if (($event['belum_absen_count'] ?? 0) > 0): ?>
                                            <div class="mt-1.5 ml-3.5">
                                                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-500/10 text-red-400 border border-red-500/20" title="Jumlah Peserta Belum Absen">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="15" y2="12"/><line x1="15" y1="8" x2="19" y2="12"/></svg>
                                                    <?= $event['belum_absen_count'] ?> Belum Absen
                                                </span>
                                            </div>
                                            <?php endif; ?>
                                        </div>
...
<script>
    ...
    function doCheckin() {
        ...
        fetch('<?= base_url('cwpa/dashboard/poin/checkin') ?>', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    ...
                    text: data.message || 'Anda mendapat <?= $poinDailyCheckin ?? 10 ?> poin!',
