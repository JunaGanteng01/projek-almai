...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-6">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6">
        ...
        <h3 class="text-2xl md:text-3xl font-bold text-accent"><?= number_format($totalEarnings, 0, ',', '.') ?> Poin</h3>
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl md:rounded-2xl p-4 md:p-6">
        ...
        <h3 class="text-2xl md:text-3xl font-bold"><?= number_format($pendingEarnings, 0, ',', '.') ?> Poin</h3>
    ...
    <div class="space-y-3">
        ...
        <div class="flex items-center justify-between p-3 bg-black/50 rounded-xl">
            ...
            <div class="text-right">
                <p class="font-bold text-accent"><?= number_format($item['earnings'], 0, ',', '.') ?> Poin</p>
            </div>
        </div>
        <?php endforeach; ?>
    </div>
        ...
        <table class="w-full min-w-[500px]">
            ...
            <tbody>
                ...
                <tr class="border-t border-white/10 hover:bg-white/5">
                    ...
                    <td class="px-4 py-3 text-accent font-bold text-sm">
                        <?php 
                        if ($trx['total'] == 0 && preg_match('/Pembayaran poin: ([\d,.]+) Poin/', $trx['notes'] ?? '', $m)) {
                            echo $m[1] . ' Poin';
                        } else {
                            echo number_format($trx['total'], 0, ',', '.') . ' Poin';
                        }
                        ?>
                    </td>
                    <td class="px-4 py-3">
