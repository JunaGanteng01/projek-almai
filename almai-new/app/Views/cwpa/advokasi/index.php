...
<div class="space-y-8">
    <!-- Back Button -->
    <div class="mb-6">
        <a href="<?= base_url('cwpa/dashboard') ?>" class="inline-flex items-center gap-2 text-gray-400 hover:text-white transition">
...
<h3 class="text-xl font-bold text-white mb-4 uppercase tracking-tight">
    PROGRAM ADVOKASI<br>
    START 18 MEI 2026 • LIVE SETIAP HARI 20.00 WIB
</h3>
           ...
           class="inline-flex items-center justify-center gap-3 bg-accent hover:bg-white hover:text-black text-black font-black uppercase tracking-widest px-8 py-4 rounded-2xl transition-all shadow-xl text-xs shadow-accent/30 w-full sm:w-auto">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    ...
                    $webinarSchedule = [
    ['day' => 'Senin', 'id' => 'senin', 'title' => 'PAHAM REGULASI & PERLINDUNGAN TRADER', 'topic' => 'Ekosistem Regulasi, Keamanan Margin, Hak & Perlindungan Trader', 'icon' => 'fa-shield-halved'],
                    ...
                    <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                <a href="<?= base_url('cwpa/dashboard/kyc') ?>" class="inline-flex items-center justify-center gap-2 px-8 py-4 bg-red-500 text-white font-black uppercase tracking-widest rounded-2xl hover:bg-red-600 transition shadow-lg shadow-red-500/20 text-xs">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($realReports as $adv): ?>
                        <div class="bg-[#121212] border border-white/5 rounded-[2rem] p-6 hover:border-accent/30 transition-all group relative overflow-hidden">
                            ...
                            <a href="<?= base_url('cwpa/dashboard/advokasi/detail/' . $adv['id']) ?>" class="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white hover:bg-white hover:text-black transition-all flex items-center justify-center gap-2 relative z-10">
...
<script>
    function handleRoadmapClick(tabId, index, totalSteps) {
        const hasPurchased = <?= ($purchase || $isCwpa) ? 'true' : 'false' ?>;
    ...
    function checkProAndRedirect() {
        ...
        if (!isPro) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('cwpa/dashboard/kyc') ?>';
                }
            });
        } else {
            window.location.href = '<?= base_url('cwpa/dashboard/advokasi/buat-laporan') ?>';
        }
    }
    ...
    function showProgramInfo() {
        Swal.fire({
            title: '<span class="text-white uppercase tracking-tighter font-bold">Info Layanan</span>',
            text: "Anda sebagai CWPA tidak perlu membeli layanan advokasi. Akses sudah terbuka secara otomatis.",
            icon: 'info',
            confirmButtonColor: '#33E818',
        ...
        activationForm.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('<?= base_url('cwpa/dashboard/advokasi/activate-license') ?>', {
                        method: 'POST',
                        body: formData,
                        headers: {
    ...
    function showSyllabus(dayId) {
        ...
        let contentHtml = `
            <div class="space-y-6 text-left">
                ...
                <div class="pt-4">
                    <a href="javascript:void(0)" onclick="checkPurchaseAndRedirect('<?= base_url('cwpa/dashboard/advokasi/materi') ?>?day=${dayId}')" class="w-full py-4 bg-white text-black font-black uppercase tracking-widest rounded-2xl hover:bg-accent transition flex items-center justify-center gap-3">
    ...
    if (testimonialForm) {
        testimonialForm.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('<?= base_url('cwpa/dashboard/advokasi/complete') ?>', {
                        method: 'POST',
                        body: formData,
                        headers: {
                            'X-Requested-With': 'XMLHttpRequest',
        ...
        Swal.fire({
            title: 'Pilih Layanan Signal',
            html: `
                <div class="flex flex-col gap-3 mt-4">
                    <a href="#" class="px-6 py-4 bg-gradient-to-r from-blue-500 to-blue-700 text-white font-bold rounded-xl hover:shadow-lg transition">Signal Plus 500</a>
                    <a href="#" class="px-6 py-4 bg-gradient-to-r from-green-500 to-green-700 text-white font-bold rounded-xl hover:shadow-lg transition">Signal Money Mall</a>
                </div>
            `,
            showConfirmButton: false,
            showCloseButton: true,
