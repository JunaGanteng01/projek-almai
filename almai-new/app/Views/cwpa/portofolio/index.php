...
<div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-[#111] p-6 rounded-xl border border-white/10">
    <div>
        <h1 class="text-2xl font-black text-white mb-1 tracking-tight">Manajemen Portofolio</h1>
        <p class="text-xs text-gray-500 uppercase tracking-widest font-bold">Monitor Performa & Bagikan Sinyal Trading</p>
    </div>
    <a href="<?= base_url('cwpa/dashboard/portofolio/create') ?>" 
       class="bg-accent hover:bg-green-400 text-black font-black px-8 py-3 rounded-xl transition-all shadow-lg hover:shadow-accent/20 text-sm flex items-center gap-2 <?= $count >= 3 ? 'opacity-50 cursor-not-allowed pointer-events-none' : '' ?>">
        <i class="fas fa-plus"></i> Tambah Akun Baru (<?= $count ?>/3)
    </a>
</div>
    ...
    <div class="mb-8 bg-accent/20 border border-accent/50 p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 animate-pulse">
        <div class="flex items-center gap-4 text-center md:text-left">
            ...
            <div>
                ...
                <p class="text-accent/80 text-xs font-bold uppercase tracking-widest">Silakan download file EA yang sudah kami kunci untuk akun Anda.</p>
            </div>
        </div>
        <a href="<?= session()->getFlashdata('download_ea') ?>" download class="bg-accent text-black font-black px-10 py-4 rounded-2xl shadow-xl hover:scale-105 transition-all text-center">
            DOWNLOAD PORTFOLIO-ALMAI.EX5
        </a>
    </div>
<?php endif; ?>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
    <?php if(!isset($accounts) || empty($accounts)): ?>
        <div class="lg:col-span-3 py-20 bg-[#111] border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center text-center">
            ...
            <a href="<?= base_url('cwpa/dashboard/portofolio/create') ?>" class="bg-accent text-black font-black px-10 py-4 rounded-2xl shadow-xl hover:scale-105 transition-all">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex gap-2">
                        <a href="<?= base_url('portofolio_eas/portofolio-almai.ex5') ?>" download class="flex-1 bg-white/5 hover:bg-white/10 text-white font-black py-3 rounded-xl text-center text-[10px] transition uppercase tracking-[0.2em] block border border-white/5">
                            <i class="fas fa-download mr-1"></i> EA File (.ex5)
                        </a>
                        <a href="<?= base_url('cwpa/dashboard/portofolio/delete/' . $acc['id']) ?>" onclick="return confirm('Hapus portofolio ini?')" class="w-12 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition flex items-center justify-center border border-red-500/10">
        ...
        <div class="space-y-6">
            <div class="flex gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs leading-relaxed opacity-70">Download file <strong>.mq5</strong> di atas, copy dan paste ke folder <span class="text-accent font-mono">MQL5/Experts</span> terminal MetaTrader 5 Anda.</p>
                </div>
            </div>
            <div class="flex gap-4">
