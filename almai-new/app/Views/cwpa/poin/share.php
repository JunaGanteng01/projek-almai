...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="<?= base_url('cwpa/dashboard/poin') ?>" class="px-4 py-2 bg-white/10 rounded-xl hover:bg-white/20 transition">
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-[#111] border border-white/10 rounded-2xl p-6">
            ...
            <form action="<?= base_url('cwpa/dashboard/poin/share/store') ?>" method="post">
                <?= csrf_field() ?>
                        ...
                        <?php foreach ($pendingSharings as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-white/10">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <form action="<?= base_url('cwpa/dashboard/poin/share/delete/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
                    ...
                    <div class="space-y-3">
                        <?php foreach ($waitingVerification as $share): ?>
                            <div class="bg-black/50 rounded-xl p-4 border border-orange-500/30">
                                ...
                                <div class="flex gap-2">
                                    <form action="<?= base_url('cwpa/dashboard/poin/share/verify/' . $share['id']) ?>" method="post" class="flex-1">
                                        <?= csrf_field() ?>
...
<script>
    ...
    function showRejectModal(id) {
        document.getElementById('rejectForm').action = '<?= base_url('cwpa/dashboard/poin/share/reject/') ?>' + id;
