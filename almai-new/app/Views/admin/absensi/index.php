...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        ...
        <a href="<?= base_url('admin/absensi/create') ?>" class="px-6 py-3 bg-accent text-black font-bold rounded-xl hover:bg-white transition flex items-center gap-2">
    ...
    <div class="bg-[#111] border border-white/10 rounded-xl overflow-hidden">
        <div class="p-6 border-b border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <form action="<?= base_url('admin/absensi') ?>" method="GET" class="flex flex-col md:flex-row gap-3">
                ...
                <?php if(!empty($filters['q']) || !empty($filters['tipe']) || !empty($filters['wpa_id']) || !empty($filters['cwpa_id'])): ?>
                    <a href="<?= base_url('admin/absensi') ?>" class="px-4 py-2 bg-white/5 text-white font-medium rounded-lg hover:bg-white/10 transition text-sm">Reset</a>
                <?php endif; ?>
            </form>
        </div>
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <?php else: ?>
                        <?php foreach ($all_activities as $item): ?>
                            <?php
                                // Superadmin (current_admin_id === null) sees all actions.
                                // NULL owner = legacy/shared record -> any admin may manage.
                                // Regular admin sees Edit/Hapus only for own records.
                                $ownerId = $item['created_by_admin_id'] ?? null;
                                $isOwner = ($current_admin_id === null) || ($ownerId === null) || ((int)$ownerId === $current_admin_id);
                            ?>
                            <tr class="hover:bg-white/5 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="<?= base_url('admin/absensi/detail/' . $item['kegiatan_type_id'] . '/' . $item['id']) ?>" class="p-2 text-green-400 hover:bg-green-400/10 rounded transition flex items-center gap-2" title="Detail">
                                        ...
                                        <?php if ($isOwner): ?>
                                        <a href="<?= base_url('admin/absensi/edit/' . $item['kegiatan_type_id'] . '/' . $item['id']) ?>" class="p-2 text-yellow-400 hover:bg-yellow-400/10 rounded transition flex items-center gap-2" title="Edit">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <a href="<?= base_url('admin/absensi/delete/' . urlencode($item['tipe']) . '/' . $item['id']) ?>" onclick="return confirm('Apakah Anda yakin ingin menghapus data absensi ini?')" class="p-2 text-red-400 hover:bg-red-400/10 rounded transition flex items-center gap-2" title="Hapus">
