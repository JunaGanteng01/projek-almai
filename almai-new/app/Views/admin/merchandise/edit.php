...
<div class="mb-6">
    <a href="<?= base_url('admin/merchandise') ?>" class="text-gray-400 hover:text-white text-sm">
...
<div class="max-w-2xl">
    <div class="bg-[#111] border border-white/10 rounded-xl p-6">
        <h2 class="text-lg font-bold mb-6">Edit Merchandise</h2>
        ...
        <form action="<?= base_url('admin/merchandise/update/' . $merchandise['id']) ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>
            
            <div class="space-y-4">
                <div>
                    <label class="block text-sm text-gray-400 mb-2">Nama Merchandise *</label>
                    <input type="text" name="name" value="<?= old('name', $merchandise['name']) ?>" required
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full bg-black border border-white/20 rounded-xl px-4 py-3 focus:border-accent focus:outline-none"><?= old('description', $merchandise['description']) ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-32 h-32 bg-gray-800 rounded-xl overflow-hidden flex items-center justify-center" id="imagePreview">
                            <?php if (!empty($merchandise['image'])): ?>
                            <img src="<?= base_url($merchandise['image']) ?>" class="w-full h-full object-cover">
                            <?php else: ?>
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 mt-2">Format: JPG, PNG. Maks 2MB. Kosongkan jika tidak ingin mengubah.</p>
                            <p class="text-xs text-gray-500 mt-1">Atau masukkan path gambar:</p>
                            <input type="text" name="image_path" value="<?= old('image_path', $merchandise['image']) ?>"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="points_required" value="<?= old('points_required', $merchandise['points_required']) ?>" required min="1"
                            class="w-full bg-black border border-white/20 rounded-xl px-4 py-3 focus:border-accent focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-sm text-gray-400 mb-2">Stok</label>
                        <input type="number" name="stock" value="<?= old('stock', $merchandise['stock']) ?>" min="0" id="stockInput"
                            class="w-full bg-black border border-white/20 rounded-xl px-4 py-3 focus:border-accent focus:outline-none"
                            <?= $merchandise['unlimited_stock'] ? 'disabled' : '' ?>>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="unlimited_stock" id="unlimited_stock" value="1" 
                        ...
                        <?= old('unlimited_stock', $merchandise['unlimited_stock']) ? 'checked' : '' ?>>
                    <label for="unlimited_stock" class="text-sm">Stok Tidak Terbatas</label>
                </div>
            ...
            <div class="flex gap-3 mt-6">
                <a href="<?= base_url('admin/merchandise') ?>" class="flex-1 py-3 border border-white/20 rounded-xl text-center hover:border-white/40 transition">
                    Batal
                </a>
