...
<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <a href="<?= base_url('admin/withdrawals') ?>" class="px-4 py-2 bg-white/5 text-gray-400 rounded-lg text-sm border <?= empty($currentStatus) ? 'border-white/20' : 'border-transparent' ?> hover:bg-white/10 transition">
            Semua
        </a>
    </div>
</div>
...
function openModal(id, currentStatus, notes) {
    ...
    form.action = `<?= base_url('admin/withdrawals/update') ?>/${id}`;
