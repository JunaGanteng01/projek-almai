...
<div class="flex flex-col md:flex-row gap-4 mb-6 items-start md:items-center justify-between">
    ...
    <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
        ...
        <a href="<?= base_url('admin/artikel/create') ?>" class="w-full sm:w-auto px-4 py-2 bg-accent text-black rounded-xl font-bold text-sm hover:bg-white transition text-center whitespace-nowrap">
                ...
                <tr class="border-t border-white/5 hover:bg-white/5">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <form action="<?= base_url('admin/artikel/approve/' . $artikel['id']) ?>" method="post" class="inline">
                                <?= csrf_field() ?>
                            ...
                            <?php endif; ?>
                            <a href="<?= base_url('admin/artikel/edit/' . $artikel['id']) ?>" class="p-2 bg-blue-500/20 text-blue-400 rounded-lg hover:bg-blue-500 hover:text-white transition" title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <form action="<?= base_url('admin/artikel/delete/' . $artikel['id']) ?>" method="post" class="inline" onsubmit="return confirm('Hapus artikel ini?')">
                                <?= csrf_field() ?>
...
function showRejectModal(id, title) {
    ...
    document.getElementById('rejectForm').action = '<?= base_url('admin/artikel/reject/') ?>' + id;
