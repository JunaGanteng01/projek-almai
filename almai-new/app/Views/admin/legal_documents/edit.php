...
<div class="max-w-5xl mx-auto">
    <!-- Page Header -->
    <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold mb-2">Edit Dokumen Legal</h1>
        <p class="text-gray-400 text-sm">Edit dokumen: <?= esc($document['title']) ?></p>
    </div>
    ...
    <form action="<?= base_url('admin/legal-documents/update/' . $document['id']) ?>" method="POST">
        <?= csrf_field() ?>
        ...
        <div class="bg-[#111] border border-white/10 rounded-xl p-6 mb-6">
            ...
            <div class="mb-6">
                ...
                <p class="text-xs text-gray-500 mt-1">Slug saat ini: <code class="bg-black/50 px-2 py-1 rounded"><?= esc($document['slug']) ?></code></p>
            </div>
            ...
            <div class="mb-6">
                ...
                <select name="type" class="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-3 text-white focus:border-accent focus:outline-none" required>
                    ...
                    <option value="perjanjian" <?= old('type', $document['type']) === 'perjanjian' ? 'selected' : '' ?>>Perjanjian</option>
                    <option value="risiko" <?= old('type', $document['type']) === 'risiko' ? 'selected' : '' ?>>Risiko</option>
                    <option value="other" <?= old('type', $document['type']) === 'other' ? 'selected' : '' ?>>Lainnya</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="content" id="contentEditor"><?= old('content', $document['content']) ?></textarea>
        ...
        <div class="flex gap-3">
            ...
            <a href="<?= base_url('admin/legal-documents') ?>" class="px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-lg transition-colors">
