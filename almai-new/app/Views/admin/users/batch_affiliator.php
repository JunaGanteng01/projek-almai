...
<div class="mb-6">
    <a href="<?= base_url('admin/users') ?>" class="inline-flex items-center text-gray-400 hover:text-white mb-4 transition text-sm">
...
<div class="grid grid-cols-1 md:grid-cols-12 gap-6 mb-6">
    <div class="md:col-span-8">
        <div class="bg-[#111] border border-white/10 rounded-2xl p-4">
            <form action="<?= base_url('admin/users/batch-affiliator') ?>" method="get" class="flex gap-2">
...
<script>
    async function updateAffiliator(userId) {
        ...
        if (result.isConfirmed) {
            ...
            try {
                const response = await fetch('<?= base_url('admin/users/process-batch-affiliator') ?>', {
                    method: 'POST',
                    headers: {
