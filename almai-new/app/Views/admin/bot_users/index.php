...
<div class="bg-[#111] border border-white/10 rounded-2xl overflow-hidden mb-6 p-4">
    <form method="get" action="" class="flex flex-col md:flex-row gap-4">
        ...
        <a href="<?= base_url('admin/bot-users/export-pdf?' . http_build_query($_GET)) ?>" class="bg-red-500 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:bg-red-600 transition-colors text-center" target="_blank"><i class="fas fa-file-pdf mr-2"></i>Export PDF</a>
        <?php if(!empty($filterSearch) || !empty($filterStatus) || !empty($filterMode)): ?>
            <a href="<?= base_url('admin/bot-users') ?>" class="bg-white/10 text-white px-6 py-2 rounded-lg text-sm font-semibold hover:bg-white/20 transition-colors text-center">Reset</a>
        <?php endif; ?>
    </form>
</div>
