...
<div class="flex justify-end mb-6">
    <a href="<?= base_url('admin/advokasi/setting') ?>" class="px-6 py-3 bg-accent text-black font-black uppercase tracking-widest rounded-2xl hover:shadow-[0_0_20px_rgba(51,232,24,0.3)] hover:scale-[1.02] transition-all text-xs">
...
<div class="bg-[#111] border border-white/10 rounded-2xl p-4 mb-6 shadow-xl">
    <form action="<?= base_url('admin/advokasi') ?>" method="get" class="flex flex-col lg:flex-row gap-3">
                ...
                <tr class="hover:bg-white/5 transition group">
                    ...
                    <td class="px-4 py-4 text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="<?= base_url('admin/advokasi/view/' . $c['id']) ?>" class="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-gray-400 hover:bg-accent hover:text-black transition">
...
<script>
    function confirmDelete(id) {
        if (confirm('Apakah Anda yakin ingin menghapus data pengaduan ini? Tindakan ini tidak dapat dibatalkan.')) {
            window.location.href = `<?= base_url('admin/advokasi/delete') ?>/${id}`;
        }
    }
</script>
