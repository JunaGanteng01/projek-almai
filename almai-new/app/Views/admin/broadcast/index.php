...
document.addEventListener('DOMContentLoaded', function() {
    ...
    roleSelector.addEventListener('change', async function() {
        ...
        try {
            const response = await fetch(`<?= base_url('admin/broadcast/get-users-by-role') ?>?role=${role}`, {
                headers: {
    ...
    broadcastForm.addEventListener('submit', function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const submitUrl = '<?= base_url("admin/broadcast/prepare") ?>';
            ...
            for (let i = 0; i < targetIds.length; i += chunkSize) {
                ...
                const processResponse = await fetch('<?= base_url("admin/broadcast/process-batch") ?>', {
                    method: 'POST',
                    headers: {
