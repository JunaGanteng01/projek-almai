...
class BappebtiPdfGenerator
{
    public function generate(array $reportData)
    {
        $html = view('keuangan/laporan/bappebti_pdf', $reportData);
        ...
        return $response->setHeader('Content-Type', 'application/pdf')
                        ->setHeader('Content-Disposition', 'inline; filename="Laporan_Bappebti_' . $reportData['tahunBerjalan'] . '.pdf"')
                        ->setBody($output);
    }
}
