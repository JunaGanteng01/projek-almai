...
class Absensi extends BaseController
{
    ...
    {
        return $this->session->get('cwpaId');
    }
    ...
    public function index()
    {
        ...
        if (empty($tipe) || $tipe == 'seminar_fgd') $addActivity($seminarModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Seminar FGD');
        if (empty($tipe) || $tipe == 'pelatihan_simulasi') $addActivity($pelatihanModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Pelatihan Simulasi');
        if (empty($tipe) || $tipe == 'signals') $addActivity($signalModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Signal');
        if (empty($tipe) || $tipe == 'konsultasi') $addActivity($konsultasiModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Konsultasi');
        if (empty($tipe) || $tipe == 'expert_advisor') $addActivity($eaModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Expert Advisor');
        if (empty($tipe) || $tipe == 'kegiatan_lainnya') $addActivity($lainnyaModel->where('kode_qr IS NOT NULL')->where('cwpa_id', $cwpaId)->findAll(), 'Kegiatan Lainnya');
        ...
        return view('cwpa/absensi/index', $data);
    }
    ...
    public function create()
    {
        ...
        // Get the WPA linked to this CWPA
        ...
        return view('cwpa/absensi/create', $data);
    }
    ...
    public function store()
    {
        ...
        if ($model->insert($data)) {
            return redirect()->to('/cwpa/dashboard/absensi')->with('success', 'Absensi berhasil dibuat. Link: ' . $linkAbsensi);
        } else {
    ...
    public function delete($kegiatanType, $id)
    {
        ...
        if (!$kegiatan || $kegiatan['cwpa_id'] != $cwpaId) {
        ...
        if ($model->delete($id)) {
            return redirect()->to('/cwpa/dashboard/absensi')->with('success', 'Data absensi berhasil dihapus.');
        }
    ...
    public function detail($kegiatanType, $id)
    {
        ...
        $kegiatan = $model->find($id);
        if (!$kegiatan || $kegiatan['cwpa_id'] != $cwpaId) {
            return redirect()->to('/cwpa/dashboard/absensi')->with('error', 'Kegiatan tidak ditemukan atau bukan milik Anda');
        }
        ...
        return view('cwpa/absensi/detail', $data);
    }
