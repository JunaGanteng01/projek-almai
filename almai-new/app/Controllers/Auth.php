...
class Auth extends BaseController
{
    ...
    {
        // Store a safe local destination before handling an existing session.
        // This also makes /login?redirect=... behave correctly if the session
        // becomes authenticated between the product click and this request.
        $redirect = $this->request->getGet('redirect');
        $redirectPath = null;
        if ($redirect) {
            $redirectPath = '/' . ltrim(str_replace(['http://', 'https://'], '', $redirect), '/');
            if (preg_match('/^[\/a-zA-Z0-9\-_\.\?=&%#]+$/', $redirectPath) && !str_starts_with($redirectPath, '//')) {
                $this->session->set('redirectAfterLogin', $redirectPath);
            } else {
                $redirectPath = null;
            }
        }
        ...
        if ($this->isLoggedIn()) {
            ...
            $destination = $redirectPath
                ?? $this->session->get('redirectAfterLogin')
                ?? \App\Models\LevelModel::getDashboardPath(\App\Models\LevelModel::levelFromRoleString($role));
        ...
        // Product purchases should land on Login. Registration remains
        // available as a tab and can still be selected explicitly.
    ...
    public function sendOtp()
    {
        ...
        try {
            ...
            $userModel = new UserModel();
            if ($affiliate_code && ! (clone $userModel)->where('code_referral', $affiliate_code)->first()) {
                return $this->response->setJSON(['success' => false, 'message' => 'Kode Referral tidak ditemukan/dihapus. Silahkan daftar dengan kode referral yang lain']);
            }
            
            if ((clone $userModel)->where('phone', $phone)->first()) {
                return $this->response->setJSON(['success' => false, 'message' => 'Nomor whatsapp sudah terdaftar']);
            }

            if ((clone $userModel)->where('email', $email)->first()) {
    ...
    public function checkEmail()
    {
        ...
        try {
            ...
            log_message('error', 'CheckEmail Fatal Error: ' . $e->getMessage());
    ...
    public function initiateRegisterV2()
    {
        ...
        try {
            ...
            $referralSource = trim($json['referral_source'] ?? 'MANUAL_INPUT');

            if (empty($name) || strlen($name) < 3) {
                return $this->response->setJSON(['success' => false, 'message' => 'Nama lengkap minimal 3 karakter.']);
            }
            ...
            if (empty($affiliatorCode)) {
                return $this->response->setJSON(['success' => false, 'message' => 'Wajib mengisi atau memilih Kode Referral.']);
            }
            ...
            $refUser = $userModel->where('referral_code', $affiliatorCode)->orWhere('username', $affiliatorCode)->orWhere('code_referral', $affiliatorCode)->first();
            if (!$refUser) {
                $wpaModel = new \App\Models\WpaModel();
                $wpa = $wpaModel->where('affiliate_code', $affiliatorCode)->orWhere('slug', strtolower($affiliatorCode))->first();
                if (!$wpa && !in_array(strtolower($affiliatorCode), ['general', 'daftar-direct', 'official', 'almai'])) {
                    return $this->response->setJSON(['success' => false, 'message' => 'Kode Referral "' . strtoupper($affiliatorCode) . '" tidak ditemukan.']);
                }
            }

            $agent = $this->request->getUserAgent();
            $deviceInfo = $agent ? $agent->getAgentString() : ($_SERVER['HTTP_USER_AGENT'] ?? 'Unknown');
            ...
            $token = $pendingModel->createPendingRegistration([
                ...
                'device_info'     => substr($deviceInfo, 0, 255),
                'ip_address'      => $this->request->getIPAddress()
            ]);

            $waMessage = "Halo ALMAI, saya ingin melakukan verifikasi registrasi akun baru.\n\n"
                . "Registration Token: {$token}\n"
                . "Nama: {$name}\n"
                . "Email: {$email}\n"
                . "Referral: {$affiliatorCode}\n\n"
                . "Mohon diproses untuk verifikasi. Terima kasih!";
            ...
            return $this->response->setJSON([
                ...
                'token'   => $token,
                'wa_url'  => $waUrl,
                'message' => 'Inisiasi registrasi berhasil. Silakan kirim pesan WhatsApp untuk verifikasi.'
            ]);
        } catch (\Throwable $e) {
    ...
    /**
     * ALMAI REGISTRATION SYSTEM v2.0 - FINAL RELEASE (POLLING & AUTO LOGIN)
     */
    public function checkRegisterStatusV2()
    {
        ...
        if ($statusInfo['status'] === 'VERIFIED') {
            ...
            if (empty($pendingRecord['user_id'])) {
                $existingUser = $userModel->where('email', $pendingRecord['email'])->first();
                ...
                if ($existingUser) {
                    ...
                    $newUser = [
                        ...
                        'phone'           => $pendingRecord['phone'] ?? null,
                        'password'        => 'Almai123',
                        'affiliator_code' => $pendingRecord['affiliator_code'],
                        'status'          => 'active',
                        'level_id'        => 1,
                        'balance'         => 0
                    ];
                    ...
                    try {
                        ...
                        $poinModel->insert([
                            ...
                            'description' => 'Bonus Pendaftaran Akun ALMAI'
                        ]);
                    } catch (\Throwable $th) {
                        log_message('error', 'Failed adding registration bonus points: ' . $th->getMessage());
                    }
                }

                $pendingModel->update($pendingRecord['id'], [
                    'user_id'    => $userId,
            ...
            $user = $userModel->find($userId);
            if ($user) {
                $role = \App\Models\LevelModel::roleStringFromLevel((int) ($user['level_id'] ?? 1));
                ...
                $this->session->setFlashdata('welcome_modal', true);

                return $this->response->setJSON([
                    'status'               => 'VERIFIED',
                    'result'               => 'SUCCESS',
                    'must_change_password' => 1,
                    'redirect'             => base_url('user/dashboard?welcome=1')
                ]);
            }
        }

        return $this->response->setJSON($statusInfo);
    }
}
