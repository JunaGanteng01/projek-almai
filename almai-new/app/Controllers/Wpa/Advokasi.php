...
class Advokasi extends BaseController
{
    public function index()
    {
        ...
        // Staff/WPA Bypass Logic (Includes CWPA level 3+)
        $isWpa = \App\Models\LevelModel::isStaffLevel((int) ($user['level_id'] ?? 0))
        ...
        } elseif ($purchase || $isWpa) {
            $eaLicenseModel = new \App\Models\EaLicenseModel();
            // Check for existing licenses
            $licenses = $eaLicenseModel->where('user_id', $userId)->where('status', 'active')->findAll();
            if (!empty($licenses)) {
                $currentStep = 3; // Move to Pelatihan Selesai if license active
            } else {
                $currentStep = 2; // On Aktivasi EA step
            }
        }
        
        $data = [
            'title' => 'Advokasi - WPA Dashboard',
            'activeMenu' => 'advokasi',
        ...
        return view('wpa/advokasi/index', $data);
    }
    ...
    public function activateLicense()
    {
        ...
        $isWpa = (in_array($userRole, ['wpa', 'cwpa', 'admin']) || ($user['level_id'] ?? 0) >= 3 || $sessionLevel >= 3);
        ...
        if ($exist) {
            return $this->response->setJSON(['success' => false, 'message' => 'Lisensi sudah diaktivasi sebelumnya.']);
        }
    ...
    public function complete()
    {
        ...
        if ($purchase || $isWpa) {
            ...
            $userName = session()->get('wpaName') ?? session()->get('userName');
    ...
    public function createReport()
    {
        ...
        $purchase = $transaksiModel->where('user_id', $userId)
                                   ...
                                   ->first();
        if (!$purchase && !$isWpa) {
            return redirect()->to('/wpa/dashboard/advokasi')->with('error', 'Anda harus mendaftar Program Advokasi terlebih dahulu.');
        }

        // Check if WPA/PRO (Level ID >= 2) - WPAs usually are
        if (($user['level_id'] ?? 0) < 2) {
             // WPAs should have access, but check just in case
        }
    ...
    public function submitReport()
    {
        ...
        if ($model->insert($complaintData)) {
            return redirect()->to('/wpa/dashboard/advokasi')->with('success', 'Laporan Anda telah berhasil dikirim.');
        }
    ...
    public function showDetail($id)
    {
        ...
        return view('wpa/advokasi/detail', [
    ...
    public function materi()
    {
        ...
        if (!$purchase && !$isWpa) {
            return redirect()->to('/wpa/dashboard/advokasi')->with('error', 'Anda harus membeli layanan Advokasi terlebih dahulu untuk mengakses materi.');
        }
        ...
        return view('wpa/advokasi/materi', [
    ...
    public function belajar()
    {
        ...
        $data = [
            'title' => 'Academy Advokasi - WPA',
            'activeMenu' => 'advokasi',
            'isPaid' => true // WPA has full access
        ];

        return view('wpa/advokasi/belajar', $data);
    }
}
