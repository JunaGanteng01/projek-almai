...
class WpaManagement extends BaseController
{
    ...
    public function index()
    {
        ...
        // ADMIN (Level 5) - only WPA whose owner registered with admin's referral code
        ...
        // Stats calculations
        // We use fresh models/builders for stats to avoid SELECT clause conflicts (only_full_group_by error)
        $buildStatsBuilder = function($status = null) use ($currentLevelId, $currentReferralCode) {
            $b = (new WpaModel())->join('users', 'users.id = wpa.user_id', 'left');
            if ($currentLevelId === \App\Models\LevelModel::LEVEL_ADMIN) {
                if (!empty($currentReferralCode)) {
        ...
        $stats = [
            ...
            'avg_rating' => $buildStatsBuilder()->selectAvg('wpa.rating')->first()['rating'] ?? 0
        ];
    ...
    public function create()
    {
        if (!$this->canWriteAdmin()) {
            return redirect()->to('/admin/wpa')->with('error', 'Akses ditolak.');
        }
    ...
    public function store()
    {
        ...
        return redirect()->to('/admin/wpa')->with('success', 'WPA berhasil ditambahkan! Password default: wpa123');
    }
    ...
    public function edit($id)
    {
        ...
        if (!$wpa) {
            return redirect()->to('/admin/wpa')->with('error', 'WPA tidak ditemukan');
        }
    ...
    public function update($id)
    {
        ...
        return redirect()->to('/admin/wpa')->with('success', 'WPA berhasil diupdate!');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->to('/admin/wpa')->with('success', 'WPA berhasil dihapus! User account dikembalikan ke level User biasa.');
    }
}
