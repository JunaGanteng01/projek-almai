...
class CwpaManagement extends BaseController
{
    ...
    public function verification()
    {
        ...
        return view('admin/cwpa/verification', [
    ...
    public function index()
    {
        ...
        // ADMIN (Level 5) - only CWPA whose owner registered with admin's referral code
        $currentLevelId = (int) session()->get('level_id');
        if ($currentLevelId === \App\Models\LevelModel::LEVEL_ADMIN) {
            $currentUser = $this->userModel->find(session()->get('userId'));
        ...
        // Stats calculations
        // We use fresh models/builders for stats to avoid SELECT clause conflicts
        $buildStatsBuilder = function($status = null) use ($currentLevelId, $currentReferralCode) {
            $b = (new CwpaModel())->join('users', 'users.id = cwpa.user_id', 'left');
            if ($currentLevelId === \App\Models\LevelModel::LEVEL_ADMIN) {
                if (!empty($currentReferralCode)) {
                    $b->where('users.affiliator_code', $currentReferralCode);
                } else {
                    $b->where('1', '0');
                }
            }
            if ($status) {
                $b->where('cwpa.status', $status);
            }
            return $b;
        };

        $stats = [
            'total' => $buildStatsBuilder()->countAllResults(),
            'active' => $buildStatsBuilder('active')->countAllResults(),
            'inactive' => $buildStatsBuilder('inactive')->countAllResults(),
        ];
    ...
    public function store()
    {
        ...
        return redirect()->to('/admin/cwpa')->with('success', 'CWPA berhasil ditambahkan!');
    }
    ...
    public function edit($id)
    {
        ...
        if (!$cwpa) {
            return redirect()->to('/admin/cwpa')->with('error', 'CWPA tidak ditemukan');
        }

        return view('admin/cwpa/edit', [
    ...
    public function update($id)
    {
        ...
        return redirect()->to('/admin/cwpa')->with('success', 'CWPA berhasil diupdate!');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->to('/admin/cwpa')->with('success', 'CWPA berhasil dihapus! User account dikembalikan ke level User biasa.');
    }
    ...
    public function getDetail($id)
    {
        ...
        if (!$submission) {
            return redirect()->to('admin/cwpa/verification')->with('error', 'Data pendaftaran tidak ditemukan');
        }
        ...
        return view('admin/cwpa/detail_verification', [
    ...
    public function promote($id)
    {
        ...
        return redirect()->to('/admin/wpa')->with('success', 'Berhasil! Data CWPA telah dipindahkan ke data WPA dan level user telah diperbarui.');
    }
}
