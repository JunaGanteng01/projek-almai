...
class Absensi extends BaseController
{
    public function index()
    {
        ...
        $data['current_admin_id'] = $this->getCurrentAdminId();
    ...
    public function store()
    {
        ...
        try {
            ...
            $db->transRollback();
            log_message('error', 'Sinkronisasi Event Gratis gagal saat membuat absensi: ' . $e->getMessage());
        ...
        if ($attendanceId && $db->transStatus()) {
            return redirect()->to('/admin/absensi')->with('success', 'Absensi berhasil dibuat. Link: ' . $linkAbsensi);
        } else {
    ...
    public function edit($kegiatanType, $id)
    {
        ...
        if (!$kegiatan) {
            return redirect()->to('/admin/absensi')->with('error', 'Kegiatan tidak ditemukan');
        }

        if ($denied = $this->assertOwnership($kegiatan)) {
            return $denied;
        }
    ...
    public function update($kegiatanType, $id)
    {
        ...
        try {
            ...
            $db->transRollback();
            log_message('error', 'Sinkronisasi Event Gratis gagal saat memperbarui absensi: ' . $e->getMessage());
        ...
        if ($updated && $db->transStatus()) {
            return redirect()->to('/admin/absensi')->with('success', 'Data absensi berhasil diperbarui.');
        } else {
    ...
    public function delete($kegiatanType, $id)
    {
        ...
        try {
            ...
            $db->transRollback();
            log_message('error', 'Sinkronisasi Event Gratis gagal saat menghapus absensi: ' . $e->getMessage());
        }
    ...
    /**
     * Return current admin id, or null for superadmin (full access).
     */
    ...
    /**
     * Ensure current admin owns the kegiatan. Superadmin bypasses.
     * Returns null if allowed, or a redirect response if denied.
     */
    private function assertOwnership(array $kegiatan)
    {
        $adminId = $this->getCurrentAdminId();
        if ($adminId === null) {
            return null;
        }
        $ownerId = $kegiatan['created_by_admin_id'] ?? null;
        // NULL owner = legacy/shared record -> any admin may manage.
        if ($ownerId === null || (int) $ownerId === $adminId) {
            return null;
        }
        return redirect()->to('/admin/absensi')->with('error', 'Anda tidak memiliki akses ke absensi ini.');
    }
    ...
    public function exportPdf($kegiatanType, $id)
    {
        ...
        return view('admin/absensi/export-pdf', $data);
    }
