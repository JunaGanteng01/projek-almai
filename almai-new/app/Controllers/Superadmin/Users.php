...
class Users extends BaseController
{
    ...
    public function exportExcel()
    {
        ...
        if ($timeFilter) {
            ...
            switch ($timeFilter) {
                ...
                case 'yesterday':
                    ...
                    break;
                case 'this_week':
                    $builder->where('YEARWEEK(created_at, 1)', date('oW'));
                    break;
                case 'this_month':
                    $builder->where('MONTH(created_at)', date('m'))
                            ->where('YEAR(created_at)', date('Y'));
        ...
        foreach ($users as $user) {
            ...
            // Abaikan user yang tidak memiliki nomor WhatsApp
            if (empty($phone)) {
                continue;
            }

            // Gunakan explicit string casting supaya nomor yang panjang tidak jadi format scientific
            $sheet->setCellValueExplicit('A' . $row, $user['name'] ?? '', \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    public function create()
    {
        if (!$this->canWriteAdmin()) {
            return redirect()->to('/superadmin/users')->with('error', 'Akses ditolak. Anda tidak memiliki izin untuk menambah user.');
        }
    ...
    public function store()
    {
        ...
        return redirect()->to('/superadmin/users')->with('success', 'User berhasil ditambahkan! Kode OTP telah dikirim via WA dan Email sebagai password.');
    }
    ...
    public function view($id)
    {
        ...
        return view('superadmin/users/view', [
    ...
    {
        if (!$this->canWriteAdmin()) {
            return redirect()->to('/superadmin/users')->with('error', 'Akses ditolak. Anda tidak memiliki izin untuk mengedit user.');
        }
    ...
    {
        if (!$this->canWriteAdmin()) {
            return redirect()->to('/superadmin/users')->with('error', 'Akses ditolak. Anda tidak memiliki izin untuk menghapus user.');
        }
        ...
        if ($user['id'] == session()->get('userId')) {
            return redirect()->to('/superadmin/users')->with('error', 'Tidak dapat menghapus akun sendiri');
        }

        // Prevent deleting admin
        if (in_array($user['level_id'], [\App\Models\LevelModel::LEVEL_ADMIN, \App\Models\LevelModel::LEVEL_SUPER_ADMIN])) {
            return redirect()->to('/superadmin/users')->with('error', 'Tidak dapat menghapus akun admin');
        }

        // Prevent deleting User PRO
        if ($user['level_id'] == \App\Models\LevelModel::LEVEL_PRO) {
            return redirect()->to('/superadmin/users')->with('error', 'Tidak dapat menghapus User PRO');
        }
        ...
        try {
            ...
            if ($db->transStatus() === false) {
                // Get the last error
                $error = $db->error();
                return redirect()->to('/superadmin/users')->with('error', 'Gagal menghapus user: ' . ($error['message'] ?? 'Unknown database error'));
            }
        } catch (\Throwable $e) {
            ...
            return redirect()->to('/superadmin/users')->with('error', 'Gagal menghapus user: ' . $e->getMessage());
        }

        return redirect()->to('/superadmin/users')->with('success', 'User dan data terkait berhasil dihapus!');
    }

    public function toggleStatus($id)
    {
        if (!$this->canWriteAdmin()) {
            return redirect()->to('/superadmin/users')->with('error', 'Akses ditolak. Anda tidak memiliki izin untuk mengubah status user.');
        }
        ...
        return redirect()->to('/superadmin/users')->with('success', 'Status user berhasil diubah!');
    }
    ...
    public function batchAffiliator()
    {
        ...
        if (session()->get('level_id') != \App\Models\LevelModel::LEVEL_SUPER_ADMIN) {
            return redirect()->to('/superadmin/users')->with('error', 'Fitur ini hanya untuk Superadmin.');
        }
        ...
        return view('superadmin/users/batch_affiliator', [
